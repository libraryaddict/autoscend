...
const SCALAR_TYPES = ["boolean", "int", "float", "string"];
...
export async function main() {
  ...
  // property -> default value (as a preference string), for scalar types only
  const settingDefaults = new Map();
  const settingResets = new Map();
  const errors = [];
  ...
  for (const file of files) {
    ...
    for (const [property, value] of Object.entries(data)) {
      ...
      if (RESET_KINDS.has(value.resets)) {
        settingResets.set(property, value.resets);
      }

      if (value.default === undefined || !SCALAR_TYPES.includes(value.type)) {
        continue;
      }
      settingDefaults.set(property, String(value.default));
    }
  }
...
`;
  ...
  // Default value (as a preference string) for each property that declares one via `default:`.
  const settingDefaultEntries = [...settingDefaults.entries()]
  ...
  // Properties that should be reset back to their default every ascension or every day.
  const settingResetEntries = [...settingResets.entries()]
    .sort(([a], [b]) => a.localeCompare(b))
    .map(
      ([property, resets]) =>
        `[${JSON.stringify(property)}, ${JSON.stringify(resets)}]`,
    );
...
export const settingDefaults: ReadonlyMap<string, string> = new Map([
  ${settingDefaultEntries.join(",\n  ")}
]);

export const settingResets: ReadonlyMap<string, ${[...RESET_KINDS].map((s) => `"${s}"`).join(" | ")}> =
  new Map([${settingResetEntries.join(", ")}]);
`;
