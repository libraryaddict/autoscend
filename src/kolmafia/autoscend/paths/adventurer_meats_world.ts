...
function amw_substatsBuyable(
  ...
  if (!meatleveling || myLevel() === 10) {
    ...
    // possibly ok to spend down if we can level up and are meatleveling. except lvl 11 bc we need to shore anyway.
    meat_reserve = 50;
  }

  if (meat_reserve >= myMeat()) {
    // no meat unreserved to spend on stats
    return 0;
  }
