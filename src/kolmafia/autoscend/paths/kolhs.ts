...
export function LM_kolhs(): boolean {
  ...
  if (LX_kolhs_school()) {
    //mandatory for first 40 adv to be spent in school
    return true;
  }
  if (LX_kolhs_yearbookCameraGet()) {
    //grab the yearbook camera if you have not already done so.
  ...
  if (LX_kolhs_yearbookCameraQuest()) {
    //gain permanent upgrades to yearbook camera
    return true;
  }
