...
export function auto_combatPeteStage1(round_1: number, enemy: Monster, text: string): string
{
	...
	if (myClass() === Class.get("Avatar of Sneaky Pete") && canSurvive$1(3.0))
	{
		...
		if (Item.get(["Sneaky Pete's leather jacket", "Sneaky Pete's leather jacket (collar popped)"]).includes(equippedItem(Slot.get("shirt"))))
		{
			maxAudience = 50;
		}
		if (canUse$2(Skill.get("Mug for the Audience")) && (myAudience() < maxAudience || disregardInstantKarma()))
		{
