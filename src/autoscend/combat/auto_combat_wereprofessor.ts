...
import { in_wereprof, is_professor, is_werewolf, wereprof_oculus } from "../paths/wereprofessor";

//defined in /autoscend/combat/auto_combat_wereprofessor.ash
export function auto_combatWereProfessorStage1(round_1: number, enemy: Monster, text: string): string
{
	if (!in_wereprof())
	{
		return "";
...
export function auto_combatWereProfessorStage4(round_1: number, enemy: Monster, text: string): string
{
...
export function auto_combatWereProfessorStage5(round_1: number, enemy: Monster, text: string): string
{
	if (!in_wereprof())
	{
		return "";
	...
	if (is_werewolf())
	{
		...
		else if (haveEquipped(Item.get("Everfull Dart Holster")) && toInt(getProperty("_dartsLeft")) > 0)
	...
	if (is_professor())
	{
		...
		else if (auto_haveCosmicBowlingBall() && canUse$4(Item.get("cosmic bowling ball")) && !enemy_physical_immune && monsterHp() < 100)
		{
