...
export function auto_combatFallOfTheDinosaursStage1(round_1: number, enemy: Monster, text: string): string
{
...
export function auto_combatFallOfTheDinosaursStage5(round_1: number, enemy: Monster, text: string): string
{
	// stage 5 = kill
	if (!in_fotd())
	{
	...
	if (dino === "archelon")
	{
		...
		if (enemy.physicalResistance >= 80 && !haveUsed(Skill.get("Silent Treatment")))
		{
			if (canUse$2(Skill.get("Implode Universe")))
			{
				return useSkill$1(Skill.get("Implode Universe"), true);
			}
			if (canUse$2(Skill.get("Silent Treatment")))
			{
	...
	if (dino === "pterodactyl")
	{ // immune to melee
		if (canUse$1(Skill.get("Snipe Pterodactyl"), false))
		{
			return useSkill$1(Skill.get("Snipe Pterodactyl"), false);
		}
