...
export function auto_combat_nanorhinoBuff(round_1: number, enemy: Monster, text: string): string
{
	...
	if (myFamiliar() !== Familiar.get("Nanorhino") || combat_status_check("nanorhino_buffed"))
	{
		return "";
	}
	...
	switch (myPrimestat())
	{
		case Stat.get("Muscle"): //get [Nanobrawny]: Muscle +25%, Weapon Damage +10, Regenerate 4-8 HP per Adventure
			if (target === Skill.none && in_glover() && canUse$2(Skill.get("Lunge Smack")) && canSurvive$1(4.0))
			{
			...
			if (target === Skill.none && stunnable(enemy) && canUse$2(Skill.get("Club Foot")) && myClass() === Class.get("Seal Clubber") && myMp() > 25)
			{
