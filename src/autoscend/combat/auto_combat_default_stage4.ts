...
export function auto_combatDefaultStage4(
  ...
  if (
    enemy === Monster.get("animated ornate nightstand") &&
    ...
    handleTracker$1(
      enemy.toString(),
      Skill.get("Get a Good Whiff of This Guy").toString(),
      "auto_sniffs",
    );
  ...
  if (
    !haveUsed$1(Item.get("Rain-Doh black box")) &&
    !in_heavyrains() &&
    toInt(getProperty("_raindohCopiesMade")) < 5 &&
    !ag_is_bodyguard()
  ) {
    if (
      enemy === Monster.get("modern zmobie") &&
  ...
  if (
    canUse$4(Item.get("Rain-Doh black box")) &&
    getProperty("auto_doCombatCopy") === "yes" &&
    enemy !== Monster.get("gourmet gourami") &&
    !ag_is_bodyguard()
  ) {
    ...
    if (toInt(getProperty("_raindohCopiesMade")) < 5) {
      handleTracker$1(
        enemy.toString(),
        Item.get("Rain-Doh black box").toString(),
        "auto_copies",
      );
  ...
  if (
    canUse$2(Skill.get("Steal Accordion")) &&
  ...
  if (
    canUse$4(Item.get("abstraction: sensation")) &&
    enemy === Monster.get("Performer of Actions")
  ) {
    //	Change +100% Moxie to +100% Init
    return useItem$1(Item.get("abstraction: sensation"));
  }
  if (
    canUse$4(Item.get("abstraction: thought")) &&
    enemy === Monster.get("Perceiver of Sensations")
  ) {
    // Change +100% Myst to +100% Items
    return useItem$1(Item.get("abstraction: thought"));
  }
  if (
    canUse$4(Item.get("abstraction: action")) &&
    enemy === Monster.get("Thinker of Thoughts")
  ) {
  ...
  if (
    myFamiliar() === Familiar.get("Stocking Mimic") &&
    round_1 < 12 &&
    canSurvive$1(1.5)
  ) {
  ...
  if (
    canUse$4(Item.get("The Big Book of Pirate Insults")) &&
    numPirateInsults() < 8 &&
  ...
  if (itemAmount(Item.get("cocktail napkin")) > 0) {
    if (
      Monster.get(["clingy pirate (female)", "clingy pirate (male)"]).includes(
        enemy,
      )
    ) {
  ...
  if (
    ...
    if (
      canUse$4(Item.get("Time-Spinner")) &&
  ...
  if (
    canUse$4(Item.get("DNA extraction syringe")) &&
  ...
  if (
    ...
    canUse$2(
      //paths that do not use MP
      Skill.get("Spring Raindrop Attack"),
    ) &&
    myMp() < 0.9 * myMaxmp()
  ) {
  ...
  if (
    fullness_left() > 0 &&
    canUse$4(Item.get("red rocket")) &&
    haveEffect(Effect.get("Everything Looks Red")) <= 0 &&
    haveEffect(Effect.get("Ready to Eat")) <= 0 &&
    canSurvive$1(5.0) &&
    myAdventures() < 100
  ) {
    if (in_plumber()) {
