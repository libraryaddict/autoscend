...
export function auto_combatDefaultStage2(
  ...
  if (
    myLocation() === Location.get("The Daily Dungeon") &&
    itemAmount(Item.get("daily dungeon malware")) > 0 &&
    auto_is_valid(Item.get("daily dungeon malware")) &&
  ...
  if (
    ...
    auto_have_skill(Skill.get("BCZ: Refracted Gaze"))
  ) {
    handleTracker$1(
      enemy.toString(),
      Skill.get("BCZ: Refracted Gaze").toString(),
      "auto_otherstuff",
    );
  ...
  if (
    extinguisherSkill !== "" &&
    haveEquipped(wrap_item(Item.get("industrial fire extinguisher"))) &&
    enemy !== Monster.get("screambat")
  ) {
  ...
  if (
    canUse$3(Item.get("glark cable"), true) &&
  ...
  if (
    ...
    handleTracker$1(
      enemy.toString(),
      Item.get("cigarette lighter").toString(),
      "auto_instakill",
    );
  ...
  if (
    ...
    if (
      !Monster.get(["dairy goat", "lobsterfrogman"]).includes(enemy) &&
      !careAboutDrops(enemy) &&
      !Location.get(["The Laugh Floor", "Infernal Rackets Backstage"]).includes(
        myLocation(),
      ) &&
      canUse$2(Skill.get("Release the Boots"))
    ) {
  ...
  if (
    ...
    itemAmount(Item.get("crumbling wooden wheel")) +
      ...
      10 &&
    canUse$2(Skill.get("Do an epic McTwist!")) &&
    !toBoolean(getProperty("_epicMcTwistUsed"))
  ) {
    handleTracker$1(
      enemy.toString(),
      Skill.get("Do an epic McTwist!").toString(),
      "auto_otherstuff",
    );
  ...
  if (
    enemy === Monster.get("mountain man") &&
    myDaycount() === 1 &&
    !auto_turbo() &&
    canUse$2(Skill.get("Do an epic McTwist!")) &&
    !toBoolean(getProperty("_epicMcTwistUsed"))
  ...
  if (auto_wantToShrunkenHead(enemy)) {
    handleTracker$1(
      enemy.toString(),
      Skill.get("Prepare to reanimate your Foe").toString(),
      "auto_otherstuff",
    );
  ...
  if (wantToClubEmBackInTime(myLocation(), enemy)) {
    if (canUse$2(Skill.get("Club 'Em Back in Time"))) {
      handleTracker$1(
        enemy.toString(),
        Skill.get("Club 'Em Back in Time").toString(),
        "auto_instakill",
      );
  ...
  if (wantToThrowGravel(myLocation(), enemy)) {
    handleTracker$1(
      enemy.toString(),
      Item.get("groveling gravel").toString(),
      "auto_instakill",
    );
    ...
    if (combatAction !== "") {
      ...
      if (indexOf(combatAction, "skill") === 0) {
        ...
        if (containsText(combatAction, ", none")) {
          ...
          handleTracker$1(
            enemy.toString(),
            toItem(substring(combatAction, 5, commapos)).toString(),
            "auto_replaces",
          );
        } else {
          handleTracker$1(
  ...
  if (
    enemy === Monster.get("tomb rat") &&
    itemAmount(Item.get("tangle of rat tails")) > 0 &&
    itemAmount(Item.get("tomb ratchet")) +
      itemAmount(Item.get("crumbling wooden wheel")) <
      10 &&
    Location.get("The Middle Chamber").fireLevel < 3
  ) {
  ...
  if (in_bugbear()) {
    if (
      enemy === Monster.get("bugbear scientist") &&
      itemAmount(Item.get("quantum nanopolymer spider web")) > 0
    ) {
      return `item ${Item.get("quantum nanopolymer spider web")}`;
    }
    if (
      enemy === Monster.get("liquid metal bugbear") &&
      itemAmount(Item.get("drone self-destruct chip")) > 0
    ) {
  ...
  if (
    ...
    Monster.get(["Racecar Bob", "Bob Racecar"]).includes(enemy) &&
  ...
  if (
    ...
    if (toInt(getProperty("desertExploration")) < 100 && !isActuallyEd()) {
      ...
      const currentDesertProgressPerTurn: number =
        ...
        (myFamiliar() === Familiar.get("Melodramedary") ? 1 : 0) +
        2 * min(1, equippedAmount(Item.get("survival knife"))) +
        equippedAmount(Item.get("UV-resistant compass")) +
        2 * equippedAmount(Item.get("ornate dowsing rod"));
      ...
      if (
        ...
        haveEffect(Effect.get("Ultrahydrated")) < fightsLeftToExplore
      ) {
    ...
    if (
      ...
      //need to do Alcove
      if (
        myLocation() === Location.get("The Defiled Alcove") &&
        haveEffect(Effect.get("Bow-Legged Swagger")) === 1
      ) {
        wantFreeKillNowEspecially = true;
      } else if (
        auto_have_skill(Skill.get("Bow-Legged Swagger")) &&
    ...
    if (
      haveEffect(Effect.get("Steely-Eyed Squint")) === 1 &&
      Location.get([
    ...
    if (
      canUse$2(Skill.get("Free-For-All")) &&
      haveEffect(Effect.get("Everything Looks Red")) === 0 &&
      ...
      handleTracker$1(
        enemy.toString(),
        Skill.get("Free-For-All").toString(),
        "auto_instakill",
      );
      loopHandlerDelayAll();
    ...
    if (
      canUse$2(Skill.get("Lightning Strike")) &&
      (wantFreeKillNowEspecially || !reserveFreekills || myLightning() >= 60)
    ) {
      handleTracker$1(
        enemy.toString(),
        Skill.get("Lightning Strike").toString(),
        "auto_instakill",
      );
      loopHandlerDelayAll();
    ...
    if (canUse$2(Skill.get("Chest X-Ray")) && auto_chestXraysRemaining() > 0) {
      if (
        ...
        handleTracker$1(
          enemy.toString(),
          Skill.get("Chest X-Ray").toString(),
          "auto_instakill",
        );
        loopHandlerDelayAll();
    ...
    if (
      canUse$2(Skill.get("Fire the Jokester's Gun")) &&
    ...
    if (
      auto_wantToBCZ(Skill.get("BCZ: Sweat Bullets")) &&
      (wantFreeKillNowEspecially || !reserveFreekills)
    ) {
      handleTracker$1(
        enemy.toString(),
        Skill.get("BCZ: Sweat Bullets").toString(),
        "auto_instakill",
      );
      loopHandlerDelayAll();
    ...
    if (
      canUse$2(Skill.get("Shattering Punch")) &&
      ...
      if (
        ...
        handleTracker$1(
          enemy.toString(),
          Skill.get("Shattering Punch").toString(),
          "auto_instakill",
        );
        loopHandlerDelayAll();
    ...
    if (
      canUse$2(Skill.get("Gingerbread Mob Hit")) &&
      ...
      handleTracker$1(
        enemy.toString(),
        Skill.get("Gingerbread Mob Hit").toString(),
        "auto_instakill",
      );
      loopHandlerDelayAll();
  ...
  if (
    canUse$4(Item.get("exploding cigar")) &&
    haveUsed(Skill.get("Duplicate"))
  ) {
