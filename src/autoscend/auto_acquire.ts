...
export function auto_buyUpTo(num: number, it: Item): boolean {
  ...
  if (
    Item.get(["Ben-Gal&trade; Balm", "hair spray"]).includes(it) &&
    !isGeneralStoreAvailable()
  ) {
    return false;
  }
...
export function npcStoreDiscountMulti(): number {
  ...
  if (
    possessEquipment(Item.get("Travoltan trousers")) &&
    auto_is_valid(Item.get("Travoltan trousers"))
  ) {
    retval -= 0.05;
  }
...
export function acquireGumItem(it: Item): boolean {
  ...
  while (
    have === itemAmount(it) &&
    myMeat() >= npcPrice(Item.get("chewing gum on a string"))
  ) {
...
export function acquireHermitItem(it: Item): boolean {
  ...
  if (
    itemAmount(Item.get("hermit permit")) === 0 &&
    myMeat() >= npcPrice(Item.get("hermit permit"))
  ) {
  ...
  if (
    it === Item.get("figurine of an ancient seal") &&
    myClass() !== Class.get("Seal Clubber")
  ) {
    return false;
  }
  if (
    it === Item.get("11-leaf clover") &&
  ...
  while (
    have === itemAmount(it) &&
    (myMeat() >= npcPrice(Item.get("chewing gum on a string")) ||
      itemAmount(Item.get("worthless trinket")) +
        itemAmount(Item.get("worthless gewgaw")) +
        itemAmount(Item.get("worthless knick-knack")) >
        0)
  ) {
    if (
      itemAmount(Item.get("worthless trinket")) +
...
export function handlePulls(day: number): number {
  ...
  if (day === 1) {
    ...
    if (!in_lol()) {
      if (
        storageAmount(Item.get("mafia thumb ring")) > 0 &&
        auto_is_valid(Item.get("mafia thumb ring"))
      ) {
        pullXWhenHaveY(Item.get("mafia thumb ring"), 1, 0);
      }
      if (
        storageAmount(Item.get("can of Rain-Doh")) > 0 &&
        auto_is_valid(Item.get("can of Rain-Doh")) &&
        pullXWhenHaveY(Item.get("can of Rain-Doh"), 1, 0)
      ) {
      ...
      if (
        storageAmount(Item.get("Buddy Bjorn")) > 0 &&
        auto_is_valid(Item.get("Buddy Bjorn")) &&
        pathHasFamiliar()
      ) {
        pullXWhenHaveY(Item.get("Buddy Bjorn"), 1, 0);
      }
      if (
        storageAmount(Item.get("Camp Scout backpack")) > 0 &&
        !possessEquipment(Item.get("Buddy Bjorn")) &&
        auto_is_valid(Item.get("Camp Scout backpack"))
      ) {
      ...
      if (!possessEquipment(Item.get("astral shirt"))) {
        ...
        if (
          storageAmount(Item.get("Sneaky Pete's leather jacket")) === 0 &&
          storageAmount(
      ...
      if (
        equippedItem(Slot.get("folder1")) ===
          Item.get("folder (tranquil landscape)") &&
        equippedItem(Slot.get("folder2")) ===
          Item.get("folder (skull and crossbones)") &&
        equippedItem(Slot.get("folder3")) ===
          Item.get("folder (Jackass Plumber)") &&
        auto_is_valid(wrap_item(Item.get("over-the-shoulder Folder Holder")))
      ) {
    ...
    if (!in_heavyrains() && pathHasFamiliar()) {
      ...
      if (
        !possessEquipment(Item.get("Snow Suit")) &&
        !possessEquipment(Item.get("astral pet sweater")) &&
        auto_is_valid(Item.get("Snow Suit"))
      ) {
        pullXWhenHaveY(Item.get("Snow Suit"), 1, 0);
      }
      const famStatEq: boolean =
        possessEquipment(Item.get("fuzzy polar bear ears")) ||
        possessEquipment(Item.get("miniature goose mask")) ||
        possessEquipment(Item.get("tiny glowing red nose"));

      if (
        !possessEquipment(Item.get("Snow Suit")) &&
        !possessEquipment(Item.get("filthy child leash")) &&
        !possessEquipment(Item.get("astral pet sweater")) &&
        !famStatEq &&
        auto_is_valid(Item.get("filthy child leash"))
      ) {
    ...
    if (
      ...
      if (
        ...
        auto_is_valid(Item.get("familiar scrapbook")) &&
        myClass() !== Class.get("Turtle Tamer")
      ) {
        // familiar scrapbook will probably be equipped in preference to Fake Washboard
      } else if (
        closetAmount(Item.get("fake washboard")) === 0 &&
        auto_is_valid(Item.get("fake washboard"))
      ) {
        pullXWhenHaveY(Item.get("fake washboard"), 1, 0);
      }
      if (
        itemAmount(Item.get("fake washboard")) === 0 &&
        closetAmount(Item.get("fake washboard")) === 0
      ) {
        if (!auto_turbo()) {
        ...
        if (toBoolean(getProperty("barrelShrineUnlocked"))) {
          putCloset(
            itemAmount(Item.get("fake washboard")),
            Item.get("fake washboard"),
          );
        }
      }
    } else {
    ...
    if (
      ...
      if (
        itemAmount(wrap_item(Item.get("Deck of Every Card"))) === 0 &&
    ...
    if (
      ...
      if (
        ...
        canEat$1(Item.get("Boris's key lime pie")) &&
        !containsText(
      ...
      if (
        ...
        canEat$1(Item.get("Sneaky Pete's key lime pie")) &&
        !containsText(
          getProperty("nsTowerDoorKeysUsed"),
      ...
      if (
        ...
        canEat$1(Item.get("Jarlsberg's key lime pie")) &&
        !containsText(
          getProperty("nsTowerDoorKeysUsed"),
    ...
    if (
      closetAmount(Item.get("fake washboard")) === 1 &&
  ...
  if (
    canPull$1(Item.get("smut orc keepsake box")) &&
...
export function LX_craftAcquireItems(): boolean {
  if (
    ...
    putCloset(
      itemAmount(Item.get("bottle of goofballs")),
      Item.get("bottle of goofballs"),
    );
  }
  ...
  if (auto_is_valid(Item.get("snow berries"))) {
    if (
      itemAmount(Item.get("snow berries")) === 3 &&
      myDaycount() === 1 &&
    ...
    if (
      itemAmount(Item.get("snow berries")) > 0 &&
      myDaycount() > 1 &&
      ...
      if (toInt(getProperty("chasmBridgeProgress")) >= bridgeGoal()) {
        ...
        if (isGuildClass()) {
          if (
            itemAmount(Item.get("snow berries")) >= 3 &&
            itemAmount(Item.get("ice harvest")) >= 3 &&
            itemAmount(Item.get("unfinished ice sculpture")) === 0
          ) {
            cliExecute("make 1 Unfinished Ice Sculpture");
          }
          if (
            itemAmount(Item.get("snow berries")) >= 2 &&
            itemAmount(Item.get("snow crab")) === 0
          ) {
  ...
  if (
    knollAvailable() &&
    itemAmount(Item.get("detuned radio")) === 0 &&
    myMeat() >= npcPrice(Item.get("detuned radio")) &&
    auto_is_valid(Item.get("detuned radio")) &&
    (!in_amw() || myLevel() >= 7)
  ) {
  ...
  if (
    ...
    auto_is_valid(Item.get("seal tooth"))
  ) {
  ...
  if (myClass() === Class.get("Turtle Tamer") && !in_wotsf()) {
    ...
    if (
      !possessEquipment(Item.get("turtle wax shield")) &&
      itemAmount(Item.get("turtle wax")) > 0
    ) {
      use(1, Item.get("turtle wax"));
    }
    if (
      haveSkill(Skill.get("Armorcraftiness")) &&
      !possessEquipment(Item.get("painted shield")) &&
      myMeat() > 3500 &&
      itemAmount(Item.get("painted turtle")) > 0 &&
    ...
    if (
      haveSkill(Skill.get("Armorcraftiness")) &&
      !possessEquipment(Item.get("spiky turtle shield")) &&
      myMeat() > 3500 &&
      itemAmount(Item.get("hedgeturtle")) > 1 &&
      itemAmount(Item.get("tenderizing hammer")) > 0
    ) {
  ...
  if (
    getPower(equippedItem(Slot.get("pants"))) < 70 &&
    !possessEquipment(Item.get("demonskin trousers")) &&
    myMeat() >= npcPrice(Item.get("pants kit")) &&
    itemAmount(Item.get("demon skin")) > 0 &&
    itemAmount(Item.get("tenderizing hammer")) > 0 &&
    knollAvailable()
  ) {
  ...
  if (
    !possessEquipment(Item.get("tighty whiteys")) &&
    myMeat() >= npcPrice(Item.get("pants kit")) &&
    itemAmount(Item.get("white snake skin")) > 0 &&
    itemAmount(Item.get("tenderizing hammer")) > 0 &&
    knollAvailable()
  ) {
    auto_buyUpTo(1, Item.get("pants kit"));
  ...
  if (
    !possessEquipment(Item.get("grumpy old man charrrm bracelet")) &&
    itemAmount(Item.get("Jolly Roger charrrm bracelet")) > 0 &&
    itemAmount(Item.get("grumpy old man charrrm")) > 0
  ) {
  ...
  if (
    !possessEquipment(Item.get("booty chest charrrm bracelet")) &&
    itemAmount(Item.get("Jolly Roger charrrm bracelet")) > 0 &&
    itemAmount(Item.get("booty chest charrrm")) > 0
  ) {
    use(1, Item.get("Jolly Roger charrrm bracelet"));
  ...
  if (
    itemAmount(Item.get("magical baguette")) > 0 &&
    !possessEquipment(Item.get("loafers"))
  ) {
  ...
  if (
    itemAmount(Item.get("magical baguette")) > 0 &&
    !possessEquipment(Item.get("bread basket"))
  ) {
    visitUrl("inv_use.php?pwd=&which=1&whichitem=8167");
    runChoice(3);
  }
  if (
    itemAmount(Item.get("magical baguette")) > 0 &&
    !possessEquipment(Item.get("breadwand"))
  ) {
    visitUrl("inv_use.php?pwd=&which=1&whichitem=8167");
    runChoice(1);
  }
  ...
  if (
    ...
    itemAmount(Item.get("demon skin")) > 0 &&
    !possessEquipment(Item.get("demonskin jacket"))
  ) {
  ...
  if (in_koe()) {
    if (
      creatableAmount(Item.get("antique accordion")) > 0 &&
      !possessEquipment(Item.get("antique accordion")) &&
      auto_predictAccordionTurns() < 10
    ) {
      retrieveItem(1, Item.get("antique accordion"));
    }
    if (
      creatableAmount(Item.get("low-pressure oxygen tank")) > 0 &&
      !possessEquipment(Item.get("low-pressure oxygen tank"))
    ) {
  ...
  if (itemAmount(Item.get("metal meteoroid")) > 0 && !in_tcrs()) {
    ...
    if (
      !possessEquipment(it) &&
      getPower(equippedItem(Slot.get("hat"))) < 140 &&
    ...
    if (
      !possessEquipment(it) &&
      !possessEquipment(Item.get("KoL Con 13 snowglobe")) &&
      toInt(getProperty("auto_beatenUpCount")) >= 5
    ) {
      const choice: number = 1 + toInt(it) - toInt(Item.get("meteortarboard"));
      let temp: string = visitUrl("inv_use.php?pwd=&which=3&whichitem=9516");
