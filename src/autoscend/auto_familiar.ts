...
export function canChangeToFamiliar(target: Familiar): boolean {
  ...
  if (
    ...
    if (
      target !== Familiar.get("Steam-Powered Cheerleader") &&
      familiarWeight(
...
export function findNonRockFamiliarInTerrarium(): Familiar {
  $_findNonRockFamiliarInTerrarium_blacklistFamiliars ??= Familiar.get([
    "Pet Rock",
    "Toothsome Rock",
    "Bulky Buddy Box",
    "Holiday Log",
...
export function findRockFamiliarInTerrarium(): Familiar {
  $_findRockFamiliarInTerrarium_petRockFamiliars ??= Familiar.get([
    "Pet Rock",
    "Toothsome Rock",
    "Bulky Buddy Box",
    "Holiday Log",
...
export function handleFamiliar$1(fam: Familiar): boolean {
  ...
  if (
    ...
    !auto_have_familiar(Familiar.get("Ms. Puck Man")) &&
    auto_have_familiar(Familiar.get("Puck Man"))
  ) {
...
function autoChooseFamiliar(place: Location): boolean {
  ...
  if (Location.get("The Black Forest") === place) {
    if (!in_bhy()) {
      if (
        ...
        canChangeToFamiliar(Familiar.get("Reassembled Blackbird"))
      ) {
        famChoice = Familiar.get("Reassembled Blackbird");
      }
    } else {
      if (
        itemAmount(Item.get("reconstituted crow")) === 0 &&
        canChangeToFamiliar(Familiar.get("Reconstituted Crow"))
      ) {
  ...
  if (
    place === Location.get("Inside the Palindome") &&
  ...
  if (
    place === Location.get("The Red Zeppelin") &&
  ...
  if (
    Location.get("The Defiled Cranny") === place &&
    auto_turbo() &&
    itemAmount(Item.get("dieting pill")) +
      toInt(getProperty("auto_dietpills")) <
      3
  ) {
  ...
  if (
    Location.get("The Defiled Nook") === place &&
  ...
  if (
    Location.get("The Obligatory Pirate's Cove") === place &&
  ...
  if (
    Location.get("The Valley of Rof L'm Fao") === place &&
  ...
  if (
    Location.get("The Castle in the Clouds in the Sky (Basement)") === place &&
    itemAmount(Item.get("heavy D")) === 0 &&
    itemAmount(Item.get("ND")) === 0 &&
    itemAmount(Item.get("Wand of Nagamar")) === 0 &&
    toBoolean(getProperty("auto_wandOfNagamar"))
  ) {
    const wantTheD: boolean =
      in_lowkeysummer() &&
      (itemAmount(Item.get("lowercase N")) > 0 ||
        Location.get("The Valley of Rof L'm Fao").turnsSpent < 11); //!possessEquipment($item[Kekekey])
    if (wantTheD) {
      famChoice = lookupFamiliarDatafile("item");
    }
  }
  ...
  if (
    Location.get("The Batrat and Ratbat Burrow") === place &&
  ...
  if (
    Location.get("The eXtreme Slope") === place &&
  ...
  if (
    Location.get("The Smut Orc Logging Camp") === place &&
    internalQuestStatus("questL09Topping") < 1 &&
    isInteger(
      (
        (Location.get("The Smut Orc Logging Camp").turnsSpent - 1) /
        20
      ).toString(),
    ) &&
    auto_haveGreyGoose()
  ...
  if (
    Location.get("The Haunted Library") === place &&
  ...
  if (
    Location.get("The Hidden Park") === place &&
  ...
  if (
    Location.get(["Wartime Frat House", "Wartime Hippy Camp"]).includes(place)
  ) {
    if (
  ...
  if (
    Location.get("The Defiled Alcove") === place &&
  ...
  if (
    ...
    wantCubeling() &&
    lookupFamiliarDatafile("item") !== Familiar.get("Gelatinous Cubeling")
  ) {
  ...
  if (
    ...
    itemAmount(Item.get("odd silver coin")) < 5 &&
    itemAmount(Item.get("grimstone mask")) === 0 &&
    Familiar.get("Grimstone Golem").dropsToday < 1 &&
...
export function wantCubeling(): boolean {
  ...
  const need_ring: boolean = !possessEquipment(
    Item.get("ring of Detect Boring Doors"),
  ); //do not try for a second one if you already have one
  const need_pole: boolean =
    !auto_haveCCSC() && itemAmount(Item.get("eleven-foot pole")) === 0;
...
export function preAdvUpdateFamiliar(place: Location): void {
  ...
  if (
    ...
    canChangeToFamiliar(Familiar.get("Cat Burglar")) &&
  ...
  if (myFamiliar() === Familiar.get("Trick-or-Treating Tot")) {
    if (Location.get(["A-Boo Peak", "The Haunted Kitchen"]).includes(place)) {
      if (
        equippedItem(Slot.get("familiar")) !==
        Item.get("li'l candy corn costume")
      ) {
  ...
  if (
    myFamiliar() === Familiar.get("Jill-of-All-Trades") &&
    possessEquipment(Item.get("LED candle"))
  ) {
