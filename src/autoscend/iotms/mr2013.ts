...
export function makeStartingSmiths(): void {
  ...
  if (
    knollAvailable() &&
    !possessEquipment(Item.get("Hairpiece On Fire")) &&
    itemAmount(Item.get("lump of Brituminous coal")) > 0
  ) {
    autoCraft(
      "smith",
...
export function oldPeoplePlantStuff(): void {
  ...
  if (myLocation() === Location.get("The Outskirts of Cobb's Knob")) {
    ...
    myClass() !== Class.get("Ed the Undying")
  ) {
    ...
    myLocation() === Location.get("Pirates of the Garbage Barges") &&
    myDaycount() === 1
  ) {
    cliExecute("florist plant Pitcher Plant");
    cliExecute("florist plant Canned Spinach");
