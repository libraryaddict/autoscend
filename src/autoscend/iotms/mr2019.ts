...
function auto_deliberate_pizza(): void {
  ...
  for (const [it, i] of Object.entries(getInventory()).map(
    ([_k, _v]) => [Item.get(_k), _v] as [Item, number],
  )) {
    if (keep_one.includes(it)) {
      if (i > 1) {
        ingredients.set(it, min(4, i - 1));
      }
    } else {
      ingredients.set(it, min(4, i));
    }
  }

  const ingredients_list: Map<number, Item> = new Map();

  for (const [it, amount] of ingredients) {
    for (let i: number = 0; i < amount; i++) {
      ingredients_list.set(ingredients_list.size, it);
    }
  }

  if (ingredients_list.size < 4) {
    auto_log_info$1("Skipping pizza planning, not enough ingredients.");
    return;
  }

  let best_plan: PizzaPlan = new PizzaPlan();
  const best_score: number = 0.0;

  for (let i: number = 0; i < 10000; i++) {
    const a: number = random(ingredients_list.size);
    const b: number = random(ingredients_list.size);
    const c: number = random(ingredients_list.size);
    const d: number = random(ingredients_list.size);

    if (a === b || a === c || a === d || b === c || b === d || c === d) {
      continue;
    }

    const speculative_plan: PizzaPlan = new PizzaPlan();
    speculative_plan.ing1 =
      ingredients_list.get(a) ?? ingredients_list.set(a, Item.none).get(a);
    speculative_plan.ing2 =
      ingredients_list.get(b) ?? ingredients_list.set(b, Item.none).get(b);
    speculative_plan.ing3 =
      ingredients_list.get(c) ?? ingredients_list.set(c, Item.none).get(c);
    speculative_plan.ing4 =
      ingredients_list.get(d) ?? ingredients_list.set(d, Item.none).get(d);

    const score: number = auto_score_pizza(speculative_plan);
    if (score > best_score) {
      best_plan = speculative_plan;
    }
  }
  auto_log_info$1(
    `Best plan:\n  ${best_plan.ing1.toString()}\n  ${best_plan.ing2.toString()}\n  ${best_plan.ing3.toString()}\n  ${best_plan.ing4.toString()}\n  `,
  );
  auto_log_info$1(`For ${auto_pizza_unclamped_advs(best_plan)} adventures.`);
}
