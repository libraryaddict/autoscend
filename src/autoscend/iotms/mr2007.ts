...
export function auto_hasNavelRing(): boolean
{
	...
	if (auto_is_valid($_auto_hasNavelRing_navelRing) && (itemAmount($_auto_hasNavelRing_navelRing) > 0 || haveEquipped($_auto_hasNavelRing_navelRing)))
	{
		return true;
	}
	// check for replica in LoL path
	$_auto_hasNavelRing_replicaNavelRing ??= Item.get("replica navel ring of navel gazing");
	return auto_is_valid($_auto_hasNavelRing_replicaNavelRing) && (itemAmount($_auto_hasNavelRing_replicaNavelRing) > 0 || haveEquipped($_auto_hasNavelRing_replicaNavelRing));
}
