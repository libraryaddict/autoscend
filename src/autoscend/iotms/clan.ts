...
import { auto_interruptCheck$1, auto_log_debug, auto_log_debug$1, auto_log_error, auto_log_info, auto_log_info$1, auto_log_warning, auto_log_warning$1, handleTracker, handleTracker$1, trim } from "../auto_util";
...
function handleFaxMonster$3(enemy: Monster, fightIt: boolean, option: CombatMacro): boolean
{
	...
	if (itemAmount(Item.get("photocopied monster")) !== 0)
	{
		if (getProperty("photocopyMonster") === enemy.toString())
		{
			...
			if (fightIt)
			{
				handleTracker$1(enemy.toString(), Item.get("deluxe fax machine").toString(), "auto_copies");
				return autoAdvBypass$2("inv_use.php?pwd&which=3&whichitem=4873", Location.get("Noob Cave"), option);
			}
			return true;
		}
		else {
...
let $_get_floundry_locations_floundryLocations: Map<Location, boolean> | undefined;
...
export function get_floundry_locations(): Map<Location, boolean>
{
	...
	if (getClanId() === $_get_floundry_locations_lastClanCheck && $_get_floundry_locations_lastCheck === myDaycount() && currentLiberation === $_get_floundry_locations_lastLiberation)
	{
...
function whitelistedClanToID(clanName: string): number
{
	...
	while (clan_matcher.find())
	{
		if (clan_matcher.group(2) === clanName)
		{
			...
			auto_log_debug$1(`Found clan ${clan_matcher.group(1)} and name: ${clan_matcher.group(2)}`);
			break;
		}
	}
	return clanID;
...
function changeClan(clanName: string): number
{
	...
	if (toClan === 0)
	{
		auto_log_warning$1("Do not have a whitelist to destination clan, can not change clans.");
		return 0;
	}
	if (!canReturn)
	{
		auto_log_warning$1("Do not have a whitelist to our own clan, can not change clans.");
		return 0;
	}
	...
	visitUrl(`showclan.php?pwd=&recruiter=1&action=joinclan&apply=Apply+to+this+Clan&confirm=on&whichclan=${toClan}`, true);
...
export function changeClan$1(toClan: number): number
{
	...
	page = visitUrl(`showclan.php?pwd=&recruiter=1&action=joinclan&apply=Apply+to+this+Clan&confirm=on&whichclan=${toClan}`, true);
...
export function isHotTubAvailable(): boolean {
	return itemAmount(Item.get("Clan VIP Lounge key")) > 0 && isUnrestricted(Item.get("Clan VIP Lounge key"));
}
...
{
	return Item.get(["glass of &quot;milk&quot;", "cup of &quot;tea&quot;", "thermos of &quot;whiskey&quot;", "Lucky Lindy", "Bee's Knees", "Sockdollager", "Ish Kabibble", "Hot Socks", "Phonus Balonus", "Flivver", "Sloppy Jalopy"]).includes(drink_1);
}
...
export function zataraSeaside(who: string): boolean
{
	...
	if (who === "susie" || who === "familiar" || who === "-1" || who === Effect.get("A Girl Named Sue").toString())
	{
		id = -1;
	}
	else if (who === "hagnk" || who === "food" || who === "booze" || who === "item" || who === "-2" || who === Effect.get("There's No N in Love").toString())
	{
		id = -2;
	}
	else if (who === "meatsmith" || who === "gear" || who === "meat" || who === "-3" || who === Effect.get("Meet the Meat").toString())
	{
		id = -3;
	}
	else if (who === "gunther" || who === "muscle" || who === "hp" || who === "-4" || who === Effect.get("Gunther Than Thou").toString())
	{
		id = -4;
	}
	else if (who === "gorgonzola" || who === "myst" || who === "mysticality" || who === "mp" || who === "-5" || who === Effect.get("Everybody Calls Him Gorgon").toString())
	{
		id = -5;
	}
	else if (who === "shifty" || who === "moxie" || who === "init" || who === "-6" || who === Effect.get("They Call Him Shifty Because...").toString())
	{
		id = -6;
	}
...
//	set_property("_clanFortuneConsultUses", get_property("_clanFortuneConsultUses").to_int() + 1);
	...
	while (attempts < 5)
	{
		...
		if (toBoolean(getProperty("auto_optimizeConsultsInRun")) && myPath() !== Path.none)
		{
			choices = "&q1=cake&q2=wonderwoman&q3=thick";
		}
		temp = visitUrl(`choice.php?pwd=&whichchoice=1278&option=1&which=1&whichid=${player}${choices}`);

		if (containsText(temp, "You can't consult Madame Zatara about your relationship with anyone else today."))
		{
		...
		if (containsText(temp, `You enter your answers and wait for ${name} to answer, so you can get your results!`))
		{
			auto_log_info("And now we play the waiting game...", "green");
			break;
		}
		if (containsText(temp, `You're already waiting on your results with ${name}.`))
		{
			auto_log_info("Results pending from prior request...", "blue");
		}
		else if (containsText(temp, "You can only consult Madame Zatara about someone in your clan."))
		{
...
function eatFancyDog(dog: string): boolean
{
	...
	$_eatFancyDog_dogReq.set("video games hot dog", Item.get("GameInformPowerDailyPro magazine"));
	...
	if (fullness_left() < ($_eatFancyDog_dogFull.get(dog) ?? $_eatFancyDog_dogFull.set(dog, 0).get(dog)))
	{
		return false;
	}
	...
	if (need > 0)
	{
		...
		{
			auto_log_warning(`Could not buy ${($_eatFancyDog_dogReq.get(dog) ?? $_eatFancyDog_dogReq.set(dog, Item.none).get(dog))} for a fancy dog. Price may have been manipulated.`, "red");
			wait(5);
			return false;
		}
	}

	if (storageAmount(($_eatFancyDog_dogReq.get(dog) ?? $_eatFancyDog_dogReq.set(dog, Item.none).get(dog))) < ($_eatFancyDog_dogAmt.get(dog) ?? $_eatFancyDog_dogAmt.set(dog, 0).get(dog)))
	{
		return false;
	}
	...
	if (($_eatFancyDog_dogAmt.get(dog) ?? $_eatFancyDog_dogAmt.set(dog, 0).get(dog)) > 0)
	{
		visitUrl(`clan_viplounge.php?preaction=hotdogsupply&hagnks=1&whichdog=${($_eatFancyDog_dogID.get(dog) ?? $_eatFancyDog_dogID.set(dog, 0).get(dog))}&quantity=${($_eatFancyDog_dogAmt.get(dog) ?? $_eatFancyDog_dogAmt.set(dog, 0).get(dog))}`);
	}
...
export function auto_floundryAction(): boolean
{
	...
	if (!toBoolean(getProperty("_floundryItemGot")) && auto_get_clan_lounge().has(Item.get("Clan Floundry")) && !inAftercore())
	{
