import { availableAmount, canadiaAvailable, canAdventure, Class, containsText, Effect, Element, equippedItem, Familiar, fullnessLimit, getLocationMonsters, getMonsters, getProperty, gnomadsAvailable, guildAvailable, haveEffect, haveFamiliar, haveOutfit, haveSkill, inHardcore, isWearingOutfit, Item, itemAmount, itemDrops, itemDropsArray, Location, Monster, monsterLevelAdjustment, myAscensions, myClass, myLevel, myPrimestat, Skill, Slot, Stat, toBoolean, toFloat, toInt, toLocation } from "kolmafia";
...
function zone_difficulty(loc: Location): generic_t
{
	...
	const mobs: Map<Monster, boolean> = new Map(Object.entries(getLocationMonsters(loc)).map(([_k, _v]) => [Monster.get(_k), _v]));
	if (mobs.size > 0)
	{
...
export function monster_to_location(target: Monster): Map<Location, boolean>
{
	...
	{ //check all locations in the game
		for (const [idx, mon] of getMonsters(loc).entries()) { if (target === mon)
		{
			retval.set(loc, true);
			break;
		} }
