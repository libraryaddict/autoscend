...
import { auto_reserveUndergroundAdventures, canBurnDelay } from "../auto_routing";
import { auto_can_equip, auto_forceNextNoncombat$1, auto_is_valid, auto_log_debug$1, auto_log_info, auto_log_info$1, auto_log_warning, fightScienceTentacle, internalQuestStatus } from "../auto_util";
...
export function L10_airship(): boolean
{
	...
	{
		auto_log_info$1("In LAR path NC178 is forced to reoccur if we skip it. Go do something else.");
		return false;
	}

	if (isBanished(Phylum.get("dude")) && !possessEquipment(Item.get("amulet of extreme plot significance")))
	{
	...
	if (isActuallyEd() && (Location.get("The Penultimate Fantasy Airship")).turnsSpent < 1)
	{
	...
	if (auto_canHabitat() && toInt(getProperty("breathitinCharges")) < 1)
	{
		...
		setProperty("auto_habitatMonster", (Monster.get("Eldritch Tentacle")).toString());
...
export function castleBasementChoiceHandler(choice: number): void
{
	...
	{ // You Don't Mess Around with Gym (The Castle in the Clouds in the Sky (Basement))
		if (internalQuestStatus("questL10Garbage") < 8 && equippedAmount(Item.get("amulet of extreme plot significance")) > 0)
		{
...
export function L10_basement(): boolean
{
	...
	else if (possessEquipment(Item.get("titanium assault umbrella")) && !in_wotsf() && !is_boris() && !auto_can_equip(Item.get("titanium assault umbrella")))
	{
		return false;
	}
	...
	{
		if (!haveSkill(Skill.get("Bendable Knees")) && itemAmount(Item.get("bottle of gregnadigne")) === 0)
		{
...
export function L10_ground(): boolean
{
	...
	if (canBurnDelay(Location.get("The Castle in the Clouds in the Sky (Ground Floor)")))
	{
		return false;
	}
...
export function L10_topFloor(): boolean
{
	...
	if (possessEquipment(Item.get("Mohawk wig")) && !auto_can_equip(Item.get("Mohawk wig")))
	{
		return false;
	}

	if (shenShouldDelayZone(Location.get("The Castle in the Clouds in the Sky (Top Floor)")))
	{
	...
	if (!possessEquipment(Item.get("Mohawk wig")) && auto_can_equip(Item.get("Mohawk wig")) && canPull$1(Item.get("Mohawk wig")))
	{
...
export function castleTopFloorChoiceHandler(choice: number): void
{
	...
	{ // Melon Collie and the Infinite Lameness (The Castle in the Clouds in the Sky (Top Floor))
		if (internalQuestStatus("questL10Garbage") < 10 && itemAmount(Item.get("drum 'n' bass 'n' drum 'n' bass record")) > 0)
		{
	...
	{ // Flavor of a Raver (The Castle in the Clouds in the Sky (Top Floor))
		if (equippedAmount(Item.get("Mohawk wig")) > 0 || internalQuestStatus("questL10Garbage") >= 10)
		{
	...
	{ // Copper Feel (The Castle in the Clouds in the Sky (Top Floor))
		if (internalQuestStatus("questL10Garbage") < 10 && itemAmount(Item.get("model airship")) > 0)
		{
			runChoice(1); // if quest not done and have model airship, complete quest
		}
		else if (internalQuestStatus("questL10Garbage") < 10 && itemAmount(Item.get("drum 'n' bass 'n' drum 'n' bass record")) > 0 || in_koe())
		{
	...
	{ // Yeah, You're for Me, Punk Rock Giant (The Castle in the Clouds in the Sky (Top Floor))
		if (internalQuestStatus("questL10Garbage") < 10 && equippedAmount(Item.get("Mohawk wig")) > 0)
		{
