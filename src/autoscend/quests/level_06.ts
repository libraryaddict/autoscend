...
import { auto_forceNextNoncombat$1, auto_log_debug, auto_log_info, auto_roughExpectedTurnsLeftToday, baseNCForcesToday, internalQuestStatus, isGuildClass, turnsUsedByRemainingNCForcesToday } from "../auto_util";
...
export function L6_friarsGetParts(): boolean
{
	...
	if (auto_have_familiar(Familiar.get("Space Jellyfish")) && toInt(getProperty("_spaceJellyfishDrops")) < 2)
	{
	...
	{
		if (!haveSkill(Skill.get("Frown Muscles")) && itemAmount(Item.get("bottle of novelty hot sauce")) === 0)
		{
	...
	{
		if (toBoolean(getProperty("auto_dakotaFanning")) && internalQuestStatus("questM16Temple") < 0)
		{
