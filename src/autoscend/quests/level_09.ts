...
import { addToMaximize, autoEquip, autoEquip$1, autoForceEquip$3, equipMaximizedGear, equipStatgainIncreasers$2, possessEquipment, resetMaximize, simMaximizeWith, simValue } from "../auto_equipment";
import { auto_famModifiers$2, auto_have_familiar, canChangeFamiliar, canChangeToFamiliar, handleFamiliar$1 } from "../auto_familiar";
...
import { auto_canCamelSpit, auto_canMapTheMonsters, auto_mapTheMonsters } from "../iotms/mr2020";
import { auto_autumnatonCanAdv, auto_hasAutumnaton, auto_haveGreyGoose } from "../iotms/mr2022";
import { auto_makeMonkeyPawWish$1 } from "../iotms/mr2023";
import { auto_canLeapBridge, auto_haveBatWings, auto_haveMayamCalendar, auto_haveSeptEmberCenser } from "../iotms/mr2024";
...
export function LX_loggingHatchet(): boolean
{
	...
	if ((Location.get("Camp Logging Camp")).turnsSpent > 0 || (Location.get("Camp Logging Camp")).combatQueue !== "" || (Location.get("Camp Logging Camp")).noncombatQueue !== "")
	{
		return false;
	}
...
export function L9_leafletQuest(): boolean
{
	...
	if (toBoolean(getProperty("leafletCompleted")) || toBoolean(getProperty("auto_leaflet_done")))
	{
		return false;
	}
	...
	if (availableAmount(Item.get("strange leaflet")) === 0)
	{
		...
		{
			auto_log_debug$1("Tried to grab a [strange leaflet] from the council and it did not work... This needs fixing. skipping for now.");
			return false;
		}
	}
...
export function finishBuildingSmutOrcBridge(): boolean
{
	...
	visitUrl(`place.php?whichplace=orc_chasm&action=bridge${toInt(getProperty("chasmBridgeProgress"))}`);
...
export function prepareForSmutOrcs(): void
{
	...
	if (isGuildClass()) {
		...
		if (setFlavour(Element.get("cold")) && canUse$2(Skill.get("Stuffed Mortar Shell")))
		{
			useSpellsInOrcCamp = true;
		}

		if (setFlavour(Element.get("cold")) && canUse$1(Skill.get("Cannelloni Cannon"), false))
		{
			useSpellsInOrcCamp = true;
		}
	...
	if (inHardcore())
	{
		...
		{
			if (!haveSkill(Skill.get("Powerful Vocal Chords")) && itemAmount(Item.get("baby oil shooter")) === 0)
			{
	...
	let need: number = (bridgeGoal() - toInt(getProperty("chasmBridgeProgress"))) / 5;
	if (need > 0)
	{
...
export function L9_chasmBuild(): boolean
{
	...
	if (auto_hasAutumnaton() && !isAboutToPowerlevel() && (Location.get("The Smut Orc Logging Camp")).turnsSpent > 0 && (fastenerCount() < bridgeGoal() || lumberCount() < bridgeGoal()))
	{
	...
	if (LX_loggingHatchet()) { // turn free, might save some adventures. May as well get it if we can.
	return true; }
...
export function L9_aBooPeak(): boolean
{
	...
	{
		auto_log_info(`A-Boo Peak (initial): ${getProperty("booPeakProgress")}`, "blue");
	...
	if (toBoolean(getProperty("auto_abooclover")) && clueAmt >= toInt(getProperty("booPeakProgress")) / 30) {
		// if you get lucky/have enough item drop to get 3 clues while getting to 90% haunted, don't waste a clover getting more.
		auto_log_info$1("We have enough A-boo clues to clear the peak, lets not waste a clover");
	...
	else if (clueCheck && toInt(getProperty("booPeakProgress")) > 2)
	{
		...
		let parrot: string = ", switch exotic parrot, switch mu, switch trick-or-treating tot";
		...
		if (blackMarketAvailable() && itemAmount(Item.get("can of black paint")) === 0 && haveEffect(Effect.get("Red Door Syndrome")) === 0 && myMeat() >= npcPrice(Item.get("can of black paint")) && !is_werewolf())
		{
			auto_buyUpTo(1, Item.get("can of black paint"));
			coldResist += 2;
			spookyResist += 2;
		}
		else if (itemAmount(Item.get("can of black paint")) > 0 && haveEffect(Effect.get("Red Door Syndrome")) === 0)
		{
			coldResist += 2;
			spookyResist += 2;
		}
		...
		if (0 === haveEffect(Effect.get("Mist Form")))
		{
			...
			else if (haveSkill(Skill.get("Spectral Awareness")) && 0 === haveEffect(Effect.get("Spectral Awareness")))
			{
				coldResist += 2;
				spookyResist += 2;
				effectiveCurrentHP -= 10;
		...
		if (itemAmount(Item.get("spooky powder")) > 0 && haveEffect(Effect.get("Spookypants")) === 0)
		{
			spookyResist = spookyResist + 1;
		}
		if (itemAmount(Item.get("ectoplasmic orbs")) > 0 && haveEffect(Effect.get("Balls of Ectoplasm")) === 0)
		{
			spookyResist = spookyResist + 1;
		}
		if (itemAmount(Item.get("black eyedrops")) > 0 && haveEffect(Effect.get("Hyphemariffic")) === 0)
		{
			spookyResist = spookyResist + 9;
		}
		if (itemAmount(Item.get("cold powder")) > 0 && haveEffect(Effect.get("Insulated Trousers")) === 0)
		{
		...
		let estimatedCold: number = toInt((13 + 25 + 50 + 125 + 250) * ((100.0 - elemental_resist_value(coldResist)) / 100.0));
		let estimatedSpooky: number = toInt((13 + 25 + 50 + 125 + 250) * ((100.0 - elemental_resist_value(spookyResist)) / 100.0));
		...
		if (min(effectiveCurrentHP, myMaxhp() + hpDifference) > totalDamage && myMp() >= mp_need)
		{
			doThisBoo = true;
		}
		if (considerHP >= totalDamage && myMp() >= mp_need && haveSkill(Skill.get("Cannelloni Cocoon")))
		{
			doThisBoo = true;
		}
		//assume min bandage HP resotred to ensure we can heal enough
		if (considerHP >= totalDamage && isActuallyEd() && itemAmount(Item.get("linen bandages")) * 20 + myHp() >= totalDamage)
		{
			doThisBoo = true;
		}
		...
		if (doThisBoo)
		{
			...
			} finally
			{
				if (getProperty("lastEncounter") !== "The Horror...")
				{
					...
					if (["Battlie Knight Ghost", "Claybender Sorcerer Ghost", "Dusken Raider Ghost", "Space Tourist Explorer Ghost", "Whatsian Commando Ghost"].includes(getProperty("lastEncounter")))
			...
			acquireFullHP();
			if (myHp() * 4 < myMaxhp() && itemAmount(Item.get("scroll of drastic healing")) > 0 && (!isActuallyEd() || !in_darkGyffte()))
			{
...
export function prepareForTwinPeak(speculative: boolean): boolean
{
	...
	if (needInit)
	{
		if (provideInitiative(40, Location.get("Twin Peak"), true, speculative) >= 40)
		{
			return true;
		}
		else {
	...
	if (needFood)
	{
		...
		{
			food_drop -= numericModifier(Familiar.get("Baby Gravy Fairy"), "Item Drop", (Servant.get("Cat")).level, Item.none);
		}
		if (food_drop < 50 && food_drop >= 20 && haveEffect(Effect.get("Brother Flying Burrito's Blessing")) === 0)
		{
			if (friarsAvailable() && !toBoolean(getProperty("friarsBlessingReceived")) && !speculative)
			{
		...
		if (food_drop < 50.0 && itemAmount(Item.get("eagle feather")) > 0 && haveEffect(Effect.get("Eagle Eyes")) === 0 && auto_is_valid(Item.get("eagle feather")))
		{
			if (!speculative) { use(1, Item.get("eagle feather")); }
			food_drop = food_drop + 20;
		}
		if (food_drop < 50.0 && itemAmount(Item.get("resolution: be happier")) > 0 && haveEffect(Effect.get("Joyful Resolve")) === 0 && auto_is_valid(Item.get("resolution: be happier")))
		{
			if (!speculative) { buffMaintain$4(Effect.get("Joyful Resolve")); }
...
export function L9_twinPeak(): boolean
{
	...
	if (hedgeTrimmersNeeded() > 0 && auto_autumnatonCanAdv(Location.get("Twin Peak")) && !isAboutToPowerlevel() && ((Location.get("Twin Peak")).turnsSpent > 0 || toInt(getProperty("twinPeakProgress")) > 0))
	...
	{
		auto_log_debug$1("Can't complete any task at the Great Overlook Lodge. Will come back to Twin Peak later");
		return false;
	}

	auto_log_info("Twin Peak", "blue");

	if (itemAmount(Item.get("rusty hedge trimmers")) === 0 && (Location.get("Twin Peak")).turnsSpent === 0 && auto_hasAutumnaton())
	{
	...
	if (toBoolean(getProperty("auto_shinningStarted")) && auto_canCamelSpit() && auto_canMapTheMonsters())
	{
		...
		if (adjustForYellowRayIfPossible(Monster.get("bearpig topiary animal")))
		{
			if (auto_mapTheMonsters())
			{
				...
				auto_log_info$1("Attemping to use Map the Monsters to Yellow Ray a Camel Spitted bearpig topiary animal. Yes that is a mouthful but lets hope it works and we get 4 rusty hedge trimmers!");
			}
		}
		else {
			return false;
	...
	if (auto_haveGreyGoose()) {
		auto_log_info$1("Bringing the Grey Goose to emit some drones to get some hedge trimmers.");
...
export function L9_oilPeak(): boolean
{
	...
	if (monsterLevelAdjustment() < 50 && myLevel() < 12 && !isAboutToPowerlevel())
	{
		return false;
	}
	...
	if (containsText(visitUrl("place.php?whichplace=highlands"), "fire3.gif"))
	{
		...
		else if (itemAmount(Item.get("bubblin' crude")) >= 12)
		{
			...
			else if (auto_is_valid(Item.get("bubblin' crude")) && creatableAmount(Item.get("jar of oil")) > 0)
			{
