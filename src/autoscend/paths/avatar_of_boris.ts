...
import { auto_change_mcd, auto_have_skill, auto_log_info, auto_log_info$1, ovenHandle } from "../auto_util";
...
export function borisAdjustML(): boolean
{
	...
	if (strong && myBuffedstat(Stat.get("Muscle")) > 100 && haveSkill(Skill.get("Pep Talk")) && haveEffect(Effect.get("Overconfident")) === 0)
	{
...
export function boris_initializeSettings(): void
{
	if (is_boris())
	{
		...
		let temp: string = visitUrl("storage.php?action=pull&whichitem1=5648&howmany1=1&pwd");
...
export function boris_buySkills(): void
{
	...
	if (haveSkill(Skill.get("Bifurcating Blow")) && haveSkill(Skill.get("Banishing Shout")) && haveSkill(Skill.get("Gourmand")))
	{
		return;
	}
...
export function borisAcquireHP(goal: number): boolean
{
	...
	while (myHp() < goal)
	{
		...
		if (myMp() === 0) { //if I reached this point with no MP I am done
		break; }
		useSkill(castAmount, Skill.get("Laugh It Off")); //multi restore HP
		if (failed_acquireMP) { //MP restore failed so we are done.
		break; }
		if (goal > myMaxhp()) { //just in case to prevent infinite loop
		break; }
	}
