import { abort, availableAmount, buyPrice, canadiaAvailable, canDrink, canEat, canEquip, canInteract, ceil, chew, Class, cliExecute, Coinmaster, containsText, creatableAmount, creatableTurns, create, dailySpecial, drink, drinksilent, eat, eatsilent, Effect, equip, equippedItem, Familiar, fullnessLimit, getProperty, gnomadsAvailable, guildStoreAvailable, haveEffect, haveSkill, historicalPrice, holiday, indexOf, inebrietyLimit, inHardcore, Item, itemAmount, mallPrice, max, min, mpCost, myAdventures, myBasestat, myClass, myDaycount, myFamiliar, myFullness, myHash, myInebriety, myLevel, myMeat, myMp, myPath, myPrimestat, mySign, mySpleenUse, npcPrice, overdrink, Path, pullsRemaining, replaceString, retrieveItem, runChoice, sellCost, setProperty, Skill, Slot, spleenLimit, splitString, Stat, stillsAvailable, substring, toBoolean, toFamiliar, toFloat, toInt, toItem, use, useFamiliar, useSkill, visitUrl } from "kolmafia";
...
export function expectedAdventuresFrom(it: Item): number
{
	function parse(): number
	{
		...
		const s: Map<number, string> = new Map(splitString(it.adventures, "-").map((_v, _i) => [_i, _v]));
...
export function consumptionProgress(): number
{
	...
	if (organs_max === 0)
	{
		...
		const used_organ_ratio: number = min(toFloat(organs_used) / toFloat(organs_max), 1);
...
{
	const organ_name: string = (action.organ === AUTO_ORGAN_STOMACH ? "fullness" : "inebriety");
...
{
	const organ: number = (it.inebriety > 0 ? AUTO_ORGAN_LIVER : AUTO_ORGAN_STOMACH);
...
function loadConsumables(_type: string, actions: Map<number, ConsumeAction>): boolean
{
	...
	for (const it of Item.get(["unremarkable duffel bag", "van key", "Knob Goblin lunchbox", "gold Boozehounds Anonymous token", "booze bindle"]))
	{
	...
	for (const it of Item.get(["Cursed Punch", "unidentified drink", "bag of QWOP", "FantasyRealm turkey leg", "FantasyRealm mead", "waffle"]))
	{
	...
	if (in_small())
	{
		...
		for (const it of Item.get(["blueberry muffin", "bran muffin", "chocolate chip muffin", "spaghetti breakfast"]))
		{
			blacklist.set(it, true);
		}
	}
	...
	if (myClass() !== Class.get("Cow Puncher"))
	{
		...
		{
			for (const it of Item.get(["tainted milk", "rotting beefsteak", "firemilk"]))
			{
				blacklist.set(it, true);
			}
		}
	...
	function add(it: Item, obtain_mode: number, howmany: number): void
	{
		for (let i: number = 0; i < howmany; i++)
		{
			...
			if (obtain_mode === AUTO_OBTAIN_PULL && !in_small())
			{
				...
				const user_desirability: number = toFloat(getProperty("auto_consumePullDesirability"));
			...
			{
				const turns_to_craft: number = creatableTurns(it, i + 1, false) - creatableTurns(it, i, false);
	...
	if (type_1 === AUTO_ORGAN_STOMACH && (itemAmount(apronKit) > 0 || canPull$1(apronKit)) && auto_is_valid(apronKit))
	{
		...
		const obtainMethod: number = (itemAmount(apronKit) > 0 ? AUTO_OBTAIN_NULL : AUTO_OBTAIN_PULL);
	...
	{ // specials are always consumable even if they would be restricted as regular consumables.
		const daily_special_limit: number = 1 + min(myMeat() / toInt(getProperty("_dailySpecialPrice")), organLeft$2() / organCost(dailySpecial()));
	...
	const cafe_stuff: Map<number, _CAFE_CONSUMABLE_TYPE> = fileAsMap(filename, [Number, ctor(_CAFE_CONSUMABLE_TYPE)]);
	if (!cafe_stuff.size)
	{
...
function auto_bestNightcap(): ConsumeAction
{
	...
	function desirability(i: number): number
	{
		...
		if (greenBeersDrinkable > 0)
		{
			...
			const greenBeerabilityBonus: number = greenBeerAdv * min(greenBeersDrinkable, max(0, 10 - (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).size));
...
function auto_overdrinkGreenBeers(): void
{
	...
	if (negativeLiver >= -10 && negativeLiver < 0)
	{
		...
		if (gnomadsAvailable())
		{
			...
			{
				const greenBeerAction: ConsumeAction = MakeConsumeAction(dailySpecial());
				...
				const daily_special_limit: number = min(beerMeat / toInt(getProperty("_dailySpecialPrice")), (inebriety_left() + 11) / dailySpecial().inebriety);
				for (let i: number = 0; i < daily_special_limit; i++)
				{
		...
		const greenbeer_limit: number = min(itemAmount(Item.get("green beer")), (inebriety_left() + 11) / (Item.get("green beer")).inebriety);
...
export function auto_findBestConsumeAction(type_1: string): ConsumeAction
{
	...
	const result_1: Map<number, boolean> = knapsack(remaining_space, space.size, space, desirability_1);
	...
	{
		const tentative_desirability_per_fill: number = (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).desirability / (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).size;
...
function auto_knapsackAutoConsume(type_1: string, simulate: boolean): boolean
{
	...
	for (const i of result_1.keys())
	{
		const name: string = consumable_name((actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)));
	...
	if (type_1 === "eat")
	{
		const applicable_seasoning: number = min(itemAmount(Item.get("Special Seasoning")), consumable_count);
...
export function auto_chewAdventures(): boolean
{
	...
	{
		for (const it of Item.get(["beastly paste", "bug paste", "cosmic paste", "oily paste", "demonic paste", "gooey paste", "elemental paste", "Crimbo paste", "fishy paste", "goblin paste", "hippy paste", "hobo paste", "indescribably horrible paste", "greasy paste", "Mer-kin paste", "orc paste", "penguin paste", "pirate paste", "chlorophyll paste", "slimy paste", "ectoplasmic paste", "strange paste", "agua de vida"]))
		{
			chooseCheapestTarget(it);
		}
	}
...
export function consumeStuff(): void
{
	...
	const edSpleenCheck: boolean = isActuallyEd() && myLevel() < 11 && spleen_left() > 0; // Ed should fill spleen first
