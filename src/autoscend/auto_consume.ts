import { Class, Coinmaster, Effect, Familiar, Item, Path, Skill, Slot, Stat, abort, availableAmount, buyPrice, canDrink, canEat, canEquip, canInteract, canadiaAvailable, ceil, chew, cliExecute, containsText, creatableAmount, creatableTurns, create, dailySpecial, drink, drinksilent, eat, eatsilent, equip, equippedItem, fullnessLimit, getProperty, gnomadsAvailable, guildStoreAvailable, haveEffect, haveSkill, historicalPrice, holiday, inHardcore, indexOf, inebrietyLimit, itemAmount, mallPrice, max, min, mpCost, myAdventures, myBasestat, myClass, myDaycount, myFamiliar, myFullness, myHash, myInebriety, myLevel, myMeat, myMp, myPath, myPrimestat, mySign, mySpleenUse, npcPrice, overdrink, pullsRemaining, replaceString, retrieveItem, runChoice, sellCost, setProperty, spleenLimit, splitString, stillsAvailable, substring, toBoolean, toFamiliar, toFloat, toInt, toItem, use, useFamiliar, useSkill, visitUrl } from "kolmafia";
...
import { equipStatgainIncreasers, equipStatgainIncreasersFor, possessEquipment } from "./auto_equipment";
import { auto_have_familiar, canChangeFamiliar, findNonRockFamiliarInTerrarium, haveSpleenFamiliar, pathAllowsChangingFamiliar, pathHasFamiliar } from "./auto_familiar";
import { auto_freeCombatsRemaining, isAboutToPowerlevel } from "./auto_powerlevel";
import { acquireMP$2 } from "./auto_restore";
import { almostRollover, auto_get_campground, auto_have_skill, auto_is_valid, auto_is_valid$2, auto_log_debug$1, auto_log_info, auto_log_info$1, auto_log_warning, auto_log_warning$1, auto_reserveAmount, auto_reserveCraftAmount, auto_turbo, banishSources, canSimultaneouslyAcquire, freeCrafts$1, handleTracker, handleTracker$1, internalQuestStatus, knapsack, meatReserve, needToConsumeForEmergencyRollover, ovenHandle, shrugAT$1 } from "./auto_util";
import { ctor, fileAsMap } from "./utils/kolmafiaUtils";
import { ConsumeAction } from "./autoscend_record";
import { canDrinkSpeakeasyDrink, drinkSpeakeasyDrink, isSpeakeasyDrink } from "./iotms/clan";
import { isClipartItem } from "./iotms/mr2011";
import { fantasyRealmAvailable } from "./iotms/mr2018";
import { auto_haveKramcoSausageOMatic, auto_sausageWanted } from "./iotms/mr2019";
...
import { auto_haveSeptEmberCenser, consumeBlackAndWhiteApronKit } from "./iotms/mr2024";
...
import { in_tcrs, tcrs_expectedAdvPerFill } from "./paths/two_crazy_random_summer";
...
export function expectedAdventuresFrom(it: Item): number
{
	...
	{
		if (!containsText(it.adventures, "-")) { return toInt(it.adventures); }
		let s: Map<number, string> = new Map(splitString(it.adventures, "-").map((_v, _i) => [_i, _v]));
		return (toInt((s.get(1) ?? s.set(1, "").get(1))) + toInt((s.get(0) ?? s.set(0, "").get(0)))) / 2.0;
	}
	let expected: number = parse();
	if (auto_have_skill(Skill.get("Saucemaven")) && saucemavenApplies(it))
	{
		if (Class.get(["Sauceror", "Pastamancer"]).includes(myClass())) { expected += 5; }
		else { expected += 3; }
	}
...
function canOde(toDrink: Item): boolean
{
	...
	if (Item.get(["steel margarita", "5-hour acrimony", "beery blood", "slap and slap again", "used beer", "shot of flower schnapps"]).includes(toDrink))
	{
		return false;
	}
...
export function autoCleanse(): boolean
{
	...
	let wantToCleanse: boolean = wantToCleanseLiver && wantToCleanseStomach; //want to cleanse both

	if (wantToCleanse && itemAmount(Item.get("spice melange")) > 0 && !toBoolean(getProperty("spiceMelangeUsed")))
	{
		handleTracker(`Cleansed with ${Item.get("spice melange")}`, "auto_otherstuff");
		return use(1, Item.get("spice melange"));
	}

	if (wantToCleanse && itemAmount(Item.get("Ultra Mega Sour Ball")) > 0 && !toBoolean(getProperty("_ultraMegaSourBallUsed")))
	{
		handleTracker(`Cleansed with ${Item.get("Ultra Mega Sour Ball")}`, "auto_otherstuff");
		return use(1, Item.get("Ultra Mega Sour Ball"));
	}

	if (wantToCleanseLiver && itemAmount(Item.get("alien plant pod")) > 0 && !toBoolean(getProperty("_alienPlantPodUsed")))
	{
		handleTracker(`Cleansed with ${Item.get("alien plant pod")}`, "auto_otherstuff");
		return use(1, Item.get("alien plant pod"));
	}

	if (wantToCleanseStomach && itemAmount(Item.get("alien animal milk")) > 0 && !toBoolean(getProperty("_alienAnimalMilkUsed")))
	{
		handleTracker(`Cleansed with ${Item.get("alien animal milk")}`, "auto_otherstuff");
...
function autoDrink$1(howMany: number, toDrink: Item, silent: boolean): boolean
{
	...
	let isSpeakeasy: boolean = isSpeakeasyDrink(toDrink);
	...
	if (toDrink === Item.get("tiny stillsuit"))
	{
		...
		let stillsuitAdvs: number = auto_expectedStillsuitAdvs();
	...
	if (canOde(toDrink) && minAdvPerDrunk(toDrink) >= 5.0 && (Familiar.get("Cooler Yeti")).experience >= 400 && (auto_haveSeptEmberCenser() && myLevel() >= 15 || (Familiar.get("Cooler Yeti")).experience > 800 || !auto_haveSeptEmberCenser()))
	{
		...
		{
			handleTracker$1((Familiar.get("Cooler Yeti")).toString(), `Double adv of ${toDrink.toString()}`, "auto_otherstuff");
			visitUrl("choice.php?pwd=&whichchoice=1560&option=2");
		}
	}

	let expectedInebriety: number = toDrink.inebriety * howMany;

	if (canOde(toDrink) && possessEquipment(Item.get("Wrist-Boy")) && myMeat() > 6500)
	{
		if (haveEffect(Effect.get("Drunk and Avuncular")) < expectedInebriety && itemAmount(Item.get("Drunk Uncles holo-record")) === 0)
		{
	...
	if (canOde(toDrink) && auto_have_skill(Skill.get("The Ode to Booze")))
	{
		...
		while (acquireMP$2(mpCost(Skill.get("The Ode to Booze")), 0) && buffMaintain$3(Effect.get("Ode to Booze"), mpCost(Skill.get("The Ode to Booze")), 1, expectedInebriety))
	...
	let it: Item = equippedItem(Slot.get("acc3"));
	if (it !== Item.get("mafia pinky ring") && itemAmount(Item.get("mafia pinky ring")) > 0 && Item.get(["bucket of wine", "Psychotic Train wine", "Sacramento wine", "stale cheer wine"]).includes(toDrink) && canEquip(Item.get("mafia pinky ring")))
	{
...
function minAdvPerDrunk(toDrink: Item): number
{
	...
	if (indexOf(toDrink.adventures, "-") < 0)
	{
		...
		minAdv = toInt(substring(toDrink.adventures, 0, indexOf(toDrink.adventures, "-")));
	}
	let size: number = toDrink.inebriety;
	return minAdv / size;
}
...
function autoDrinkCafe(howmany: number, id: number): boolean
{
	...
	if (!gnomadsAvailable()) { return false; }

	equipStatgainIncreasersFor(toItem(id));

	let name: string = cafeDrinkName(id);
	for (let i: number = 0; i < howmany; i++)
	{
		// TODO: What if we run out of meat?
		visitUrl("cafe.php?cafeid=2");
		visitUrl(`cafe.php?pwd=${myHash()}&phash=${myHash()}&cafeid=2&whichitem=${id}&action=CONSUME!`);
...
function autoEatCafe(howmany: number, id: number): boolean
{
	...
	if (!canadiaAvailable()) { return false; }

	equipStatgainIncreasersFor(toItem(id));

	let name: string = cafeFoodName(id);
	for (let i: number = 0; i < howmany; i++)
	{
		// TODO: What if we run out of meat?
		visitUrl("cafe.php?cafeid=1");
		visitUrl(`cafe.php?pwd=${myHash()}&phash=${myHash()}&cafeid=1&whichitem=${id}&action=CONSUME!`);
...
export function autoChew(howMany: number, toChew: Item): boolean
{
	...
	let retval: boolean = chew(howMany, toChew);
...
function autoEat$1(howMany: number, toEat: Item, silent: boolean): boolean
{
	...
	let expectedFullness: number = toEat.fullness * howMany;
	...
	{
		if (haveEffect(Effect.get("Record Hunger")) < expectedFullness && itemAmount(Item.get("The Pigs holo-record")) === 0)
		{
	...
	while (howMany > 0)
	{
		buffMaintain$3(Effect.get("Song of the Glorious Lunch"), 10, 1, toEat.fullness);
		if (auto_get_campground().has(Item.get("portable Mayo Clinic")) && myMeat() - meatReserve() > npcPrice(Item.get("Mayoflex")) && getProperty("mayoInMouth") === "" && auto_is_valid(Item.get("portable Mayo Clinic")))
		{
		...
		if (itemAmount(Item.get("mini kiwi aioli")) > 0 || itemAmount(Item.get("mini kiwi")) >= 5 && itemAmount(Item.get("mini kiwi aioli")) === 0)
		{ //use mini kiwi aioli if we got one from the mini kiwi
			...
			if (toEat.fullness > 3)
			{
				...
				handleTracker(`Used ${Item.get("mini kiwi aioli")} for ${toEat}`, "auto_otherstuff");
			}
		}
		...
		if (retval)
		{
			...
			if (toInt(getProperty("auto_dietpills")) > 0)
			{
				...
				setProperty("auto_dietpills", (toInt(getProperty("auto_dietpills")) - 1).toString());
			}
			if (detail !== "")
			{
...
export function acquireMilkOfMagnesiumIfUnused(useAdv: boolean): boolean
{
	...
	if (toBoolean(getProperty("_milkOfMagnesiumUsed")) || toBoolean(getProperty("milkOfMagnesiumActive")))
	{
		return true;
	}
	if (fullnessLimit() === 0)
	...
	ovenHandle();
	if (itemAmount(Item.get("glass of goat's milk")) > 0 && haveSkill(Skill.get("Advanced Saucecrafting")))
	{
		if (itemAmount(Item.get("scrumptious reagent")) === 0 && myMp() >= mpCost(Skill.get("Advanced Saucecrafting")))
		{
...
{
	if (toBoolean(getProperty("_milkOfMagnesiumUsed")) || toBoolean(getProperty("milkOfMagnesiumActive")) || itemAmount(Item.get("milk of magnesium")) < 1)
	{
		return false;
	}
...
function minAdvPerFull(toEat: Item): number
{
	...
	if (indexOf(toEat.adventures, "-") < 0)
	{
		...
		minAdv = toInt(substring(toEat.adventures, 0, indexOf(toEat.adventures, "-")));
	}
	let size: number = toEat.fullness;
	if (size === 0) { //Fullness data isn't in Mafia yet for the item in question
	return 0; }
	return minAdv / size;
}
	...
	if (minAdvPerFull(toEat) > minAdvPerFullForDietPill())
	{
		...
		if (fullness_left() >= 2 * toEat.fullness && spleen_left() >= 3)
		{
			...
			if (itemAmount(pill) > 0)
			{
				...
				setProperty("auto_dietpills", (toInt(getProperty("auto_dietpills")) + 1).toString()); //Track how many dieting pills we have consumed this ascension
				return chew(1, pill);
			}
		}
		return false;
...
function canDrink$2(toDrink: Item, checkValidity: boolean): boolean
{
	...
	{
		return sellCost(Coinmaster.get("Jarlsberg's Cosmic Kitchen"), toDrink).size > 0;
	}
	if (in_nuclear() && toDrink.inebriety > 1)
	{
		return false;
	}
	if (in_darkGyffte() !== Item.get(["vampagne", "dusty bottle of blood", "Red Russian", "mulled blood", "bottle of Sanguiovese"]).includes(toDrink))
	{
		return false;
	}
	if (in_kolhs())
	{
		if (!(Item.get(["can of the cheapest beer", "bottle of fruity &quot;wine&quot;", "single swig of vodka", "fountain 'soda'", "stepmom's booze", "steel margarita"]).includes(toDrink)))
		{
			return false;
		}
	}
	if (in_lta())
	{
		let martinis: Map<number, Item> = bondDrinks();
		let found: boolean = false;
		for (let [idx, it] of martinis)
		{
			if (it === toDrink)
			{
				found = true;
	...
	if (is_werewolf())
	{
		...
		if (Item.get(["Champagne Shimmy", "Charleston Choo-Choo", "Marltini", "Mysterious Stranger", "Strong, Silent Type", "Velvet Veil"]).includes(toDrink))
		{
			return false;
		}
	}
...
function canEat$2(toEat: Item, checkValidity: boolean): boolean
{
	...
	if (is_jarlsberg())
	{
		return sellCost(Coinmaster.get("Jarlsberg's Cosmic Kitchen"), toEat).size > 0;
	}
	...
	if (in_darkGyffte() !== Item.get(["blood-soaked sponge cake", "blood roll-up", "blood snowcone", "actual blood sausage", "bloodstick"]).includes(toEat))
	{
		return false;
	}
	if (in_zombieSlayer())
	{
		return Item.get(["crappy brain", "decent brain", "good brain", "boss brain", "hunter brain", "brains casserole", "fricasseed brains", "steel lasagna"]).includes(toEat);
	}
...
export function consumptionProgress(): number
{
	...
	if (organs_max === 0)
	{
		...
		let used_organ_ratio: number = min(toFloat(organs_used) / toFloat(organs_max), 1);
...
function MakeConsumeAction(it: Item): ConsumeAction
{
	...
	let adv: number = expectedAdventuresFrom(it);
...
function autoConsume(action: ConsumeAction): boolean
{
	...
	if (action.organ === AUTO_ORGAN_LIVER && action.it !== Item.get("tiny stillsuit"))
	{
...
function loadConsumables(_type: string, actions: Map<number, ConsumeAction>): boolean
{
	...
	for (let it of Item.get(["unremarkable duffel bag", "van key", "Knob Goblin lunchbox", "gold Boozehounds Anonymous token", "booze bindle"]))
	{
	...
	let type_1: number = 0;
	if (_type === "eat") { type_1 = AUTO_ORGAN_STOMACH; }
	else if (_type === "drink") { type_1 = AUTO_ORGAN_LIVER; }
	else { return false; }

	function canConsume(it: Item, checkValidity: boolean): boolean
	{
		if (checkValidity && isClipartItem(it) && !auto_is_valid$2(Skill.get("Summon Clip Art")) && !canInteract())
		{
			//workaround for this mafia bug https://kolmafia.us/threads/g-lovers-clip-art-create-function-failure.26007/
			return false;
		}
		return (type_1 === AUTO_ORGAN_STOMACH ? canEat$2(it, checkValidity) : canDrink$2(it, checkValidity));
	}
	...
	let pullables: Map<Item, number> = new Map();
	let small_owned: Map<Item, number> = new Map();
	let buyables: Map<Item, number> = new Map();
	let large_owned: Map<Item, number> = new Map();
	let craftables: Map<Item, number> = new Map();

	let blacklist: Map<Item, boolean> = new Map();
	let craftable_blacklist: Map<Item, boolean> = new Map();

	for (let it of Item.get(["Cursed Punch", "unidentified drink", "bag of QWOP", "FantasyRealm turkey leg", "FantasyRealm mead", "waffle"]))
	{
		blacklist.set(it, true);
	}
	if (toBoolean(getProperty("auto_dontConsumeLegendPizzas")) && !in_small() || auto_turbo() && toInt(getProperty("cyrptCrannyEvilness")) > 0)
	{
	...
	if (in_small())
	{
		...
		for (let it of Item.get(["blueberry muffin", "bran muffin", "chocolate chip muffin", "spaghetti breakfast"]))
		{
			blacklist.set(it, true);
		}
	}
	if (is_professor() || is_werewolf() && toInt(getProperty("wereProfessorTransformTurns")) < 50)
	{
	...
	if (itemAmount(Item.get("wet stunt nut stew")) === 0 && !possessEquipment(Item.get("Mega Gem")) && !isActuallyEd())
	{
	...
	if (LX_doingPirates() && internalQuestStatus("questM12Pirate") <= 2 && itemAmount(Item.get("hot wing")) < 4)
	{
	...
	if (myClass() !== Class.get("Cow Puncher"))
	{
		...
		if (myBasestat(Stat.get("Moxie")) > 30 || myPrimestat() === Stat.get("Muscle") && myBasestat(Stat.get("Muscle")) > 30)
		{
			for (let it of Item.get(["tainted milk", "rotting beefsteak", "firemilk"]))
			{
				blacklist.set(it, true);
			}
		}
	...
	add_mutex_craftables(new Map([[Item.get("perfect cosmopolitan"), true], [Item.get("perfect old-fashioned"), true], [Item.get("perfect mimosa"), true], [Item.get("perfect dark and stormy"), true], [Item.get("perfect paloma"), true], [Item.get("perfect negroni"), true]]));
	...
	{
		if (!(blacklist.has(it)) && canConsume$1(it) && organCost(it) > 0 && (it.fullness === 0 || it.inebriety === 0) && auto_is_valid(it))
		{
			let value_allowed: boolean = historicalPrice(it) < auto_getConsumablePriceLimit() || Item.get(["blueberry muffin", "bran muffin", "chocolate chip muffin"]).includes(it) && itemAmount(it) > 0 && myPath() !== Path.get("Grey You"); //Grey You should not even get to here if ever supported but it consumes the tin so blocked just in case
									//muffins are expensive but renewable

			if (!value_allowed) { continue; }
			if ((it === Item.get("astral pilsner") || it === Item.get("Cold One") || it === Item.get("astral hot dog")) && myLevel() < 11) { continue; }
			if (it === Item.get("spaghetti breakfast") && (myLevel() < 11 || myFullness() > 0 || toBoolean(getProperty("_spaghettiBreakfastEaten")))) { continue; }
			if (it === Item.get("Pizza of Legend") && toBoolean(getProperty("pizzaOfLegendEaten"))) { continue; }
			if (it === Item.get("Calzone of Legend") && toBoolean(getProperty("calzoneOfLegendEaten"))) { continue; }
			if (it === Item.get("Deep Dish of Legend") && toBoolean(getProperty("deepDishOfLegendEaten"))) { continue; }
			...
			if (it === Item.get("spaghetti breakfast")) { howmany = 1; }
			if (itemAmount(it) > 0 && organCost(it) <= 5)
			{
				small_owned.set(it, min(max(itemAmount(it) - auto_reserveAmount(it), 0), howmany));
			}
			...
			if (itemAmount(it) > 0 && organCost(it) > 5)
			{
				large_owned.set(it, min(max(itemAmount(it) - auto_reserveAmount(it), 0), howmany));
			}
			if (!(craftable_blacklist.has(it)) && creatableAmount(it) > 0)
			{
				craftables.set(it, min(howmany, max(0, creatableAmount(it) - auto_reserveCraftAmount(it))));
			}
			if (it === Item.get("pheromone cocktail") && itemAmount(it) > 0 && banishSources() - itemAmount(it) < 3)
			{
				potentialTurnGain.set(it, 2);
			}
			...
			if (!isSpeakeasyDrink(it) && canPull$1(it))
			{
				...
				else { //pullable amount here was coded before the change to daily limit of 1 pull each
	...
	if (missingHeroKeys > 0 && !toBoolean(getProperty("auto_dontConsumeKeyLimePies")))
	{
		...
		function considerNextPie(): void
		{
			...
			if (!wantBorisPie && myPrimestat() === Stat.get("Muscle") && itemAmount(Item.get("Boris's key")) === 0 && auto_is_valid(Item.get("Boris's key lime pie")))
				{ wantBorisPie = true; }
			else if (!wantJarlsbergPie && myPrimestat() === Stat.get("Mysticality") && itemAmount(Item.get("Jarlsberg's key")) === 0 && auto_is_valid(Item.get("Jarlsberg's key lime pie")))
				{ wantJarlsbergPie = true; }
			else if (!wantPetePie && itemAmount(Item.get("Sneaky Pete's key")) === 0 && auto_is_valid(Item.get("Sneaky Pete's key lime pie")))
				{ wantPetePie = true; }
			else if (!wantJarlsbergPie && itemAmount(Item.get("Jarlsberg's key")) === 0 && auto_is_valid(Item.get("Jarlsberg's key lime pie")))
				{ wantJarlsbergPie = true; }
			else if (!wantBorisPie && itemAmount(Item.get("Boris's key")) === 0 && auto_is_valid(Item.get("Boris's key lime pie")))
				{ wantBorisPie = true; }
		}
		...
		keysObtainableFromDailyDungeon = (toBoolean(getProperty("dailyDungeonDone")) ? 0 : 1);
		if (keysObtainableFromDailyDungeon > 0)
		{
			if (itemAmount(Item.get("daily dungeon malware")) > 0 && !toBoolean(getProperty("_dailyDungeonMalwareUsed")) && toInt(getProperty("_lastDailyDungeonRoom")) < 14 && !in_pokefam())
			{
	...
	function add(it: Item, obtain_mode: number, howmany: number): void
	{
		for (let i: number = 0; i < howmany; i++)
		{
			...
			if (obtain_mode === AUTO_OBTAIN_PULL && !in_small())
			{
				...
				(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).desirability -= 5.0;
				let user_desirability: number = toFloat(getProperty("auto_consumePullDesirability"));
				if (user_desirability > 0.0)
				{
					(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).desirability = -user_desirability;
				}
			}
			if (type_1 === AUTO_ORGAN_STOMACH && auto_is_valid(Item.get("Special Seasoning")))
			{
				(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).desirability += min(1.0, toFloat(itemAmount(Item.get("Special Seasoning"))) * it.fullness / fullness_left());
			}
			if (obtain_mode === AUTO_OBTAIN_NULL)
			{
				if (it === Item.get("spaghetti breakfast"))
				{
					if (toBoolean(getProperty("_spaghettiBreakfastEaten")) || myFullness() > 0)
					{
						(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).desirability -= 50;
					}
					else {
						auto_log_info$1("Spaghetti Breakfast available, we should eat that first.");
						(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).desirability += 50;
					}
				}
				else if (Item.get(["blueberry muffin", "bran muffin", "chocolate chip muffin"]).includes(it))
				{
					if (myFullness() === 0 && myLevel() < 13 && toFloat(getProperty("auto_consumeMinAdvPerFill")) <= (actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).adventures / (actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).size)
					{
						if (!inHardcore() && myLevel() >= 12 && auto_have_skill(Skill.get("Saucemaven")))
						{
							//eating it at 12 would probably mean having to pull something smaller than a hi mein and missing out on Saucemaven?
						}
						else {
							auto_log_info$1(`${it.toString()} available, we should eat that first.`);
							(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).desirability += 50;
						}
					}
				}
			...
			{
				let turns_to_craft: number = creatableTurns(it, i + 1, false) - creatableTurns(it, i, false);
				(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).desirability -= turns_to_craft;
			}
			else {
				if (i === 0 && (it === Item.get("Boris's key lime pie") && wantBorisPie || it === Item.get("Jarlsberg's key lime pie") && wantJarlsbergPie || it === Item.get("Sneaky Pete's key lime pie") && wantPetePie))
				{
					auto_log_info$1(`If we ate a ${it} we could skip getting a fat loot token...`);
					(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).desirability += keyLimePieDesirabilityBonus;
				}
				if (i === 0 && it === Item.get("pheromone cocktail") && (potentialTurnGain.get(it) ?? potentialTurnGain.set(it, 0).get(it)) > 0)
				{
					(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).desirability += (potentialTurnGain.get(it) ?? potentialTurnGain.set(it, 0).get(it));
				}
			}
			(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).howtoget = obtain_mode;
		}
	}

	for (let [it, howmany] of pullables)
	{
		add(it, AUTO_OBTAIN_PULL, howmany);
	}
	for (let [it, howmany] of small_owned)
	{
		add(it, AUTO_OBTAIN_NULL, howmany);
	}
	for (let [it, howmany] of buyables)
	{
		add(it, AUTO_OBTAIN_BUY, howmany);
	}
	for (let [it, howmany] of large_owned)
	{
		add(it, AUTO_OBTAIN_NULL, howmany);
	}
	for (let [it, howmany] of craftables)
	{
		add(it, AUTO_OBTAIN_CRAFT, howmany);
	}
	// Add still suit if we are looking to drink
	if (type_1 === AUTO_ORGAN_LIVER && auto_hasStillSuit() && !in_kolhs() && !in_small())
	{
		let size: number = 1;
		let adv: number = toFloat(auto_expectedStillsuitAdvs());
		actions.set(actions.size, new ConsumeAction(Item.get("tiny stillsuit"), 0, size, adv, adv, AUTO_ORGAN_LIVER, AUTO_OBTAIN_NULL));
	}
	// Add black and white apron if we are looking to eat
	let apronKit: Item = Item.get("Black and White Apron Meal Kit");
	if (type_1 === AUTO_ORGAN_STOMACH && (itemAmount(apronKit) > 0 || canPull$1(apronKit)) && auto_is_valid(apronKit))
	{
		let size: number = 3;
		let adv: number = 12.0;
		let obtainMethod: number = (itemAmount(apronKit) > 0 ? AUTO_OBTAIN_NULL : AUTO_OBTAIN_PULL);
		actions.set(actions.size, new ConsumeAction(apronKit, 0, size, adv, adv, AUTO_ORGAN_STOMACH, obtainMethod));
	}
	// Now, to load cafe consumables. This has some TCRS-specific code.

	if (type_1 === AUTO_ORGAN_LIVER && !gnomadsAvailable()) { return false; }
	if (type_1 === AUTO_ORGAN_STOMACH && !canadiaAvailable()) { return false; }
	// Add daily special
	...
	{ // specials are always consumable even if they would be restricted as regular consumables.
		let daily_special_limit: number = 1 + min(myMeat() / toInt(getProperty("_dailySpecialPrice")), organLeft$2() / organCost(dailySpecial()));
		for (let i: number = 0; i < daily_special_limit; i++)
		{
			let n: number = actions.size;
			actions.set(n, MakeConsumeAction(dailySpecial()));
			(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).cafeid = toInt(dailySpecial());
			(actions.get(n) ?? actions.set(n, new ConsumeAction()).get(n)).it = Item.none;
		}
	}
	...
	if (!in_tcrs())
	{
		...
		if (type_1 === AUTO_ORGAN_LIVER)
		{
			...
			let limit: number = 1 + min(myMeat() / 100, inebriety_left() / 3);
			for (let i: number = 0; i < limit; i++)
			{
				let size: number = 3;
				let adv: number = 11.0 / 3.0;
				actions.set(actions.size, new ConsumeAction(Item.none, -3, size, adv, adv, AUTO_ORGAN_LIVER, AUTO_OBTAIN_NULL));
			}
		}
		if (type_1 === AUTO_ORGAN_STOMACH)
		{
			...
			for (let i: number = 0; i < limit; i++)
			{
				let size: number = 3;
				let adv: number = 3.5;
				actions.set(actions.size, new ConsumeAction(Item.none, -1, size, adv, adv, AUTO_ORGAN_LIVER, AUTO_OBTAIN_NULL));
			}
			// As Jus Gezund Heit
			...
			for (let i: number = 0; i < limit; i++)
			{
				let size: number = 4;
				let adv: number = 5.0;
				actions.set(actions.size, new ConsumeAction(Item.none, -2, size, adv, adv, AUTO_ORGAN_LIVER, AUTO_OBTAIN_NULL));
			}
			// As Jus Gezund Heit
	...
	let cafe_stuff: Map<number, _CAFE_CONSUMABLE_TYPE> = fileAsMap(filename, [Number, ctor(_CAFE_CONSUMABLE_TYPE)]);
	if (!cafe_stuff.size)
	{
		abort(`Something went wrong while trying to load ${filename}. Maybe run 'tcrs load'?`);
	}
	for (let [i, r] of cafe_stuff)
	{
		// Always-available cafe items have item ids -1, -2, -3
		if (i >= -3 && r.space > 0)
		{
			let limit: number = 1 + min(myMeat() / 100, organLeft$2() / r.space);
			for (let j: number = 0; j < limit; j++)
			{
				let size: number = r.space;
				let adv: number = r.space * tcrs_expectedAdvPerFill(r.quality);
				actions.set(actions.size, new ConsumeAction(Item.none, -3, size, adv, adv, AUTO_ORGAN_LIVER, AUTO_OBTAIN_NULL));
			}
		}
	}
...
function auto_bestNightcap(): ConsumeAction
{
	...
	let have_ode: boolean = auto_have_skill(Skill.get("The Ode to Booze"));
	let greenBeersDrinkable: number = 0;
	let greenBeerAdv: number = 0;
	if (containsText(holiday(), "St. Sneaky Pete's Day") && gnomadsAvailable() && dailySpecial() === Item.get("green beer"))
	{
		let disposableBeerMeat: number = max(0, myMeat() - meatReserve());
		greenBeersDrinkable = min(ceil(10.0 / (Item.get("green beer")).inebriety), disposableBeerMeat / toInt(getProperty("_dailySpecialPrice")));
		if (greenBeersDrinkable > 0)
		{
			...
			greenBeerAdv = toInt(expectedAdventuresFrom(Item.get("green beer")) + ((have_ode ? (Item.get("green beer")).inebriety : 0)));
		}
	}

	function desirability(i: number): number
	{
		let ret: number = (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).desirability;
		if (have_ode) { ret += (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).size; }
		if (greenBeersDrinkable > 0)
		{
			//on Sneaky Pete's Day smaller drink action leaves more space for green beers
			let greenBeerabilityBonus: number = greenBeerAdv * min(greenBeersDrinkable, max(0, 10 - (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).size));
			ret += greenBeerabilityBonus;
			if ((actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).it === Item.get("astral pilsner"))
	...
	for (let i: number = 1; i < actions.size; i++)
	{
		...
		if (desirability(i) === current_best_desirability && historicalPrice((actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).it) >= historicalPrice((actions.get(best) ?? actions.set(best, new ConsumeAction()).get(best)).it))
		{
...
function auto_overdrinkGreenBeers(): void
{
	...
	let start_fam: Familiar = myFamiliar();
	if (auto_have_familiar(Familiar.get("Stooper")) && start_fam !== Familiar.get("Stooper") && pathAllowsChangingFamiliar())
	...
	let negativeLiver: number = inebriety_left();
	if (negativeLiver >= -10 && negativeLiver < 0)
	{
		...
		if (gnomadsAvailable())
		{
			...
			{
				let greenBeerAction: ConsumeAction = MakeConsumeAction(dailySpecial());
				greenBeerAction.cafeid = toInt(dailySpecial());
				greenBeerAction.it = Item.none;
				let beerMeat: number = myMeat() - ((in_wotsf() ? meatReserve() : 0)); //extra advs are almost always worth more, but meat is hard to get in wotsf
				let daily_special_limit: number = min(beerMeat / toInt(getProperty("_dailySpecialPrice")), (inebriety_left() + 11) / dailySpecial().inebriety);
				for (let i: number = 0; i < daily_special_limit; i++)
				{
		...
		let greenbeer_limit: number = min(itemAmount(Item.get("green beer")), (inebriety_left() + 11) / (Item.get("green beer")).inebriety);
...
export function auto_drinkNightcap(): void
{
	...
	if (toBoolean(getProperty("auto_skipNightcap")) || toBoolean(getProperty("auto_limitConsume")))
	{
		return;
	}
	if (in_darkGyffte())
	...
	function overdrunk(): boolean
	{
		...
		{
			if (Familiar.get("Stooper") === myFamiliar() && inebriety_left() < 0) { //stooper is current familiar and overdrunk
			return true; }
			else if (inebriety_left() < -1) { //stooper not current familiar. but will be overdrunk even if switching to it
			return true; }
		}
		else if (inebriety_left() < 0) { //we can not use stooper and are overdrunk
		return true; }
		return false;
	}
	if (overdrunk())
	...
	let target: ConsumeAction = auto_bestNightcap();
...
export function auto_findBestConsumeAction(type_1: string): ConsumeAction
{
	...
	{
		if (type_1 === "eat") { return fullness_left(); }
		if (type_1 === "drink")
		{
	...
	let remaining_space: number = organLeft();

	let desirability_1: Map<number, number> = new Map();
	let space: Map<number, number> = new Map();
	for (let i: number = 0; i < actions.size; i++)
	{
		desirability_1.set(i, (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).desirability);
		space.set(i, (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).size);
	}

	let result_1: Map<number, boolean> = knapsack(remaining_space, space.size, space, desirability_1);

	let best_desirability_per_fill: number = 0.0;
	let best_adv_per_fill: number = 0.0;
	let best: number = -1;
	for (let i of result_1.keys())
	{
		let tentative_desirability_per_fill: number = (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).desirability / (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).size;
		if (tentative_desirability_per_fill > best_desirability_per_fill)
		{
			best_desirability_per_fill = tentative_desirability_per_fill;
			best_adv_per_fill = (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).adventures / (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).size;
			best = i;
		}
	}
...
function auto_findBestConsumeAction$1(): ConsumeAction
{
	...
	{
		auto_log_info$1("Will not drink to maintain pool skill for Haunted Billiards room.");
		considerDrink = false;
		if (fullness_left() === 0)
		{
			auto_log_warning$1("Need to drink as no fullness is available, pool skill will suffer.");
			considerDrink = true;
		}
	}

	let bestFoodAction: ConsumeAction = auto_findBestConsumeAction("eat");
...
export function auto_autoConsumeOne(action: ConsumeAction): boolean
{
	...
	if (Item.get(["Boris's key lime pie", "Jarlsberg's key lime pie", "Sneaky Pete's key lime pie"]).includes(action.it))
	{
		...
		if (action.howtoget === AUTO_OBTAIN_PULL) { best_adv_per_fill = toInt((action.desirability + 5) / action.size); }
		else if (action.howtoget === AUTO_OBTAIN_CRAFT) { best_adv_per_fill = toInt((action.desirability + 1) / action.size); }
		else { best_adv_per_fill = toInt(action.desirability / action.size); }
	}
	...
	{
		auto_log_warning$1(`auto_autoConsumeOne: Will not consume, min adventures per full ${best_adv_per_fill} is less than auto_consumeMinAdvPerFill ${getProperty("auto_consumeMinAdvPerFill")}`);
		return false;
	}
...
function auto_knapsackAutoConsume(type_1: string, simulate: boolean): boolean
{
	...
	{
		if (type_1 === "eat") { return fullness_left(); }
		if (type_1 === "drink") { return inebriety_left(); }
		abort(`Unrecognized organ type: should be 'eat' or 'drink', was ${type_1}`);
		return 0;
	}
	if (organLeft$1() === 0) { return false; }

	let actions: Map<number, ConsumeAction> = new Map();
	loadConsumables(type_1, actions);
	// Non-pulled, non-cafe consumables
	let normal_consumables: Map<Item, number> = new Map();

	let remaining_space: number = organLeft$1();
	...
	let organ_name: string = (type_1 === "eat" ? "fullness" : "inebriety");
	auto_log_info(`Knapsack ${type_1} plan for ${remaining_space} ${organ_name}:`, "blue");
	let total_adv: number = 0.0;
	let consumable_count: number = 0;
	let sum_space: number = 0;
	for (let i of result_1.keys())
	{
		let name: string = consumable_name((actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)));
		if ((actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).it !== Item.none && (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).howtoget !== AUTO_OBTAIN_PULL)
		{
			normal_consumables.set((actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).it, (normal_consumables.get((actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).it) ?? 0) + 1);
		}
		consumable_count++;
		sum_space += (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).size;
		auto_log_info(to_pretty_string((actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i))), "blue");
		total_adv += (actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)).adventures;
	}
	if (type_1 === "eat")
	{
		let applicable_seasoning: number = min(itemAmount(Item.get("Special Seasoning")), consumable_count);
		auto_log_info(`(+${applicable_seasoning} from special seasoning (${itemAmount(Item.get("Special Seasoning"))} available)`, "blue");
	...
	if (result_1.size === 0)
	{
		auto_log_warning(`Couldn't find a way of finishing off our ${type_1} space exactly.`, "red");
		return false;
	}

	if (!canSimultaneouslyAcquire(normal_consumables))
	{
		auto_log_warning(`It looks like I can't simultaneously get everything that I want to ${type_1}. I'll wait and see if I get unconfused - otherwise, please ${type_1} manually.`, "red");
		return false;
	}
	...
	for (let i of result_1.keys())
	{
		if (!autoPrepConsume((actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i))))
		{
			abort(`Unexpectedly couldn't prep ${to_debug_string((actions.get(i) ?? actions.set(i, new ConsumeAction()).get(i)))}`);
		}
	}
	...
	let pre_adventures: number = myAdventures();
	...
	auto_log_info(`Expected ${total_adv} adventures, got ${myAdventures() - pre_adventures}`, "blue");
	return true;
}
...
export function auto_chewAdventures(): boolean
{
	...
	let liver_check: boolean = myInebriety() < inebrietyLimit() && !in_kolhs(); //kolhs has special drinking. liver often unfilled
	if (liver_check || myFullness() < fullnessLimit() || myAdventures() > max(10, 1 + auto_advToReserve()) && !almostRollover())
	{
	...
	{
		for (let it of Item.get(["beastly paste", "bug paste", "cosmic paste", "oily paste", "demonic paste", "gooey paste", "elemental paste", "Crimbo paste", "fishy paste", "goblin paste", "hippy paste", "hobo paste", "indescribably horrible paste", "greasy paste", "Mer-kin paste", "orc paste", "penguin paste", "pirate paste", "chlorophyll paste", "slimy paste", "ectoplasmic paste", "strange paste", "agua de vida"]))
		{
			chooseCheapestTarget(it);
		}
	}

	let oldSpleenUse: number = mySpleenUse();
	if (target !== Item.none)
	{
...
export function auto_breakfastCounterVisit(): boolean {
	if (itemAmount(Item.get("earthenware muffin tin")) > 0 || !toBoolean(getProperty("_muffinOrderedToday")) && Item.get(["blueberry muffin", "bran muffin", "chocolate chip muffin", "earthenware muffin tin"]).includes(toItem(getProperty("muffinOnOrder")))) {
		auto_log_info$1("Going to the breakfast counter to grab/order a breakfast muffin.");
		...
		if (!toBoolean(getProperty("_muffinOrderedToday")) && itemAmount(Item.get("earthenware muffin tin")) > 0) {
			auto_log_info$1("Ordering a bran muffin for tomorrow to keep you regular.");
...
export function still_targetToOrigin(target: Item): Item
{
	...
	{
		return ($_still_targetToOrigin_originNeeded.get(target) ?? $_still_targetToOrigin_originNeeded.set(target, Item.none).get(target));
	}
	else {
		auto_log_debug$1(`still_targetToOrigin failed to lookup the item [${target}]`);
	}
	return Item.none;
}
...
export function stillReachable(): boolean
{
	...
	return guildStoreAvailable() && Class.get(["Accordion Thief", "Disco Bandit"]).includes(myClass());
}
...
export function distill(target: Item): boolean
{
	...
	if (!stillReachable())
	{
		auto_log_warning$1(`distill(item target) tried to create [${target}] but Nash Crosby's Still is not reachable`);
		return false;
	}
	if (stillsAvailable() === 0)
	{
		auto_log_warning$1(`distill(item target) tried to create [${target}] but Nash Crosby's Still is out of uses`);
		return false;
	}
	let start_amount: number = itemAmount(target);
	create(1, target); //use the still to create target
	if (start_amount + 1 === itemAmount(target))
	{
		return true;
	}
	auto_log_warning$1(`distill(item target) mysteriously failed to create [${target}]`);
	return false;
}
...
export function prepare_food_xp_multi(): boolean
{
	...
	if (have_fireworks_shop() && !in_wereprof() && haveEffect(
	// don't want to use in WereProfessor
	Effect.get("Ready to Eat")) <= 0 && auto_is_valid(Item.get("red rocket")))
	{
		if (itemAmount(Item.get("red rocket")) === 0 && myMeat() > npcPrice(Item.get("red rocket")))
		{
	...
	equipStatgainIncreasers(new Map([[Stat.get("Muscle"), true], [Stat.get("Mysticality"), true], [Stat.get("Moxie"), true]]), true);
...
export function consumeStuff(): void
{
	...
	if (in_amw())
	{
		if (almostRollover() && needToConsumeForEmergencyRollover() || myAdventures() < max(10, 1 + auto_advToReserve()))
		{
			amw_buyAdv();
		}
		return;
	...
	let edSpleenCheck: boolean = isActuallyEd() && myLevel() < 11 && spleen_left() > 0; // Ed should fill spleen first

	if (myAdventures() < max(10, 1 + auto_advToReserve()) && fullness_left() > 0 && is_boris())
	{
		borisDemandSandwich(true);
	}
	// guilty sprouts provide big stats. Eat if powerleveling
	if (isAboutToPowerlevel() && auto_is_valid(Item.get("guilty sprout")) && canEat$1(Item.get("guilty sprout")) && myLevel() < 13 && !in_tcrs())
	{
		// attempt to eat spaghetti breakfast as can only be eaten as the first food of the day
		if (myLevel() >= 11 && myFullness() === 0 && !toBoolean(getProperty("_spaghettiBreakfastEaten")))
		{
	...
	if (myAdventures() < max(10, 1 + auto_advToReserve()) && !edSpleenCheck || almostRollover() && needToConsumeForEmergencyRollover())
	{
		// always unequip stooper as only useful for roll over
		if (myFamiliar() === Familiar.get("Stooper") && toFamiliar(getProperty("auto_100familiar")) !== Familiar.get("Stooper") && pathAllowsChangingFamiliar())
		{ //check path allows changing of familiars
			useFamiliar(findNonRockFamiliarInTerrarium());
		}

		let bestAction: ConsumeAction = auto_findBestConsumeAction$1();
...
export function shouldUseSpleenForLowPriority(): boolean
{
	...
	spleen_likely_to_use += (Item.get("dieting pill")).spleen * availableAmount(Item.get("dieting pill"));
