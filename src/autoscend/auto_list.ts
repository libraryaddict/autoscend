...
function List(): Map<number, Familiar> {
  const retval: Map<number, Familiar> = new Map();
  return retval;
}
...
function ListFind$3(
  ...
  const retval: Map<number, Familiar> = List$6(list);
  let at: number = idx;
...
function ListRemove$3(
  list: Map<number, Familiar>,
  what: Familiar,
  idx: number,
): Map<number, Familiar> {
  const retval: Map<number, Familiar> = List$6(list);
  for (const [at, el] of retval) {
    if (el === what && at >= idx) {
      retval.delete(at);
    }
  }
...
function ListInsertAt$1(
  ...
  while (shift > idx) {
    retval.set(
      shift,
      retval.get(shift - 1) ??
        retval.set(shift - 1, Familiar.none).get(shift - 1),
    );
    shift = shift - 1;
  }
  retval.set(idx, what);
...
function ListInsertInorder$1(
  ...
  if (
    what.toString() <
    (retval.get(0) ?? retval.set(0, Familiar.none).get(0)).toString()
  ) {
...
function main$auto_list(): void {
  ...
  while (index < list.size) {
    auto_log_info(
      `${index}: ${list.get(index) ?? list.set(index, Familiar.none).get(index)}: ${ListFind$2(list, list.get(index) ?? list.set(index, Familiar.none).get(index))}`,
      "blue",
    );
    index = index + 1;
  }
  auto_log_info(
    `${3}: ${list.get(3) ?? list.set(3, Familiar.none).get(3)}: ${ListFind$3(list, list.get(3) ?? list.set(3, Familiar.none).get(3), 1)}`,
    "blue",
  );
  auto_log_info(
    `${3}: ${list.get(3) ?? list.set(3, Familiar.none).get(3)}: ${ListFind$3(list, list.get(3) ?? list.set(3, Familiar.none).get(3), 2)}`,
    "blue",
  );
  auto_log_info(
    `${3}: ${list.get(3) ?? list.set(3, Familiar.none).get(3)}: ${ListFind$3(list, list.get(3) ?? list.set(3, Familiar.none).get(3), 3)}`,
    "blue",
  );
  auto_log_info(
    `${3}: ${list.get(3) ?? list.set(3, Familiar.none).get(3)}: ${ListFind$3(list, list.get(3) ?? list.set(3, Familiar.none).get(3), 4)}`,
    "blue",
  );
}
// start of int[int]
...
function intList(): Map<number, number> {
  const retval: Map<number, number> = new Map();
  return retval;
}
...
function ListFind$5(
  ...
  const retval: Map<number, number> = List$7(list);
  let at: number = idx;
  while (at < retval.size) {
...
function ListInsertAt$2(
  ...
  while (shift > idx) {
    retval.set(
      shift,
      retval.get(shift - 1) ?? retval.set(shift - 1, 0).get(shift - 1),
    );
    shift = shift - 1;
  }
  retval.set(idx, what);
...
function ListInsertAt$3(
  ...
  while (shift > idx) {
    retval.set(
      shift,
      retval.get(shift - 1) ?? retval.set(shift - 1, Item.none).get(shift - 1),
    );
    shift = shift - 1;
  }
  retval.set(idx, what);
...
function ListInsertInorder$3(
  ...
  if (
    what.toString() <
    (retval.get(0) ?? retval.set(0, Item.none).get(0)).toString()
  ) {
...
function ListInsertAt(
  ...
  while (shift > idx) {
    retval.set(
      shift,
      retval.get(shift - 1) ??
        retval.set(shift - 1, Effect.none).get(shift - 1),
    );
    shift = shift - 1;
  }
  retval.set(idx, what);
...
function ListInsertInorder(
  ...
  if (
    what.toString() <
    (retval.get(0) ?? retval.set(0, Effect.none).get(0)).toString()
  ) {
...
function ListFind$9(
  ...
  const retval: Map<number, Location> = List$10(list);
  let at: number = idx;
  while (at < retval.size) {
...
function ListInsertAt$4(
  ...
  while (shift > idx) {
    retval.set(
      shift,
      retval.get(shift - 1) ??
        retval.set(shift - 1, Location.none).get(shift - 1),
    );
    shift = shift - 1;
  }
  retval.set(idx, what);
...
function ListInsertInorder$4(
  ...
  if (
    what.toString() <
    (retval.get(0) ?? retval.set(0, Location.none).get(0)).toString()
  ) {
...
function auto_sortedByModifier$2(
...
function auto_sortedByModifier$4(
...
function auto_sortedByModifier$5(
  map: Map<Effect, number>,
  m: Modifier,
  high_to_low: boolean,
): Map<number, Effect> {
...
function auto_sortedByModifier$6(
  map: Map<Effect, boolean>,
  m: Modifier,
): Map<number, Effect> {
  return auto_sortedByModifier$7(map, m, true);
}

function auto_sortedByModifier$7(
  map: Map<Effect, boolean>,
  m: Modifier,
  high_to_low: boolean,
): Map<number, Effect> {
  const int_map: Map<Effect, number> = new Map();
  for (const entry of map.keys()) {
    int_map.set(entry, (int_map.get(entry) ?? 0) + 1);
  }
