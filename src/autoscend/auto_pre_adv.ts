import { abort, appearanceRates, canEat, ceil, changeMcd, Class, cliExecute, combatRateModifier, containsText, creatableAmount, create, currentMcd, Effect, Element, equip, equippedAmount, equippedItem, experienceBonus, Familiar, getMonsters, getProperty, haveEffect, haveEquipped, inebrietyLimit, inHardcore, initiativeModifier, isUnrestricted, isWearingOutfit, Item, itemAmount, itemDropModifier, Location, meatDropModifier, Modifier, Monster, monsterAttack, monsterElement, monsterLevelAdjustment, monsterPhylum, mpCost, myBasestat, myBuffedstat, myClass, myCompanion, myFamiliar, myFury, myHp, myInebriety, myLevel, myLocation, myMaxfury, myMaxhp, myMaxmp, myMeat, myMp, myPath, myPrimestat, myServant, mySessionAdv, mySign, mySoulsauce, myThrall, myTurncount, npcPrice, numericModifier, putCloset, retrieveItem, setProperty, Skill, Slot, splitString, Stat, Thrall, toBoolean, toFamiliar, toInt, toLocation, toMonster, use, useFamiliar, useSkill } from "kolmafia";
import { speculative_pool_skill } from "../autoscend";
import { auto_buyUpTo } from "./auto_acquire";
import { buffMaintain$3, buffMaintain$4 } from "./auto_buff";
import { addBonusToMaximize, addToMaximize, auto_equipFreekill, auto_saveEquipped, autoEquip, autoEquip$1, autoForceEquip$1, autoForceEquip$3, autoOutfit, equipMaximizedGear, equipOverrides, equipStatgainIncreasers$1, equipStatgainIncreasers$2, possessEquipment, possessOutfit$1, removeFromMaximize, simMaximizeWith$1, simValue } from "./auto_equipment";
import { auto_famWeight$2, canChangeFamiliar, canChangeToFamiliar, handleFamiliar$1, is100FamRun, pathHasFamiliar, preAdvUpdateFamiliar } from "./auto_familiar";
import { provideInitiative$2, provideItem$2, provideMeat$2 } from "./auto_providers";
import { acquireHP, acquireMP$2, mp_regen, uneffect } from "./auto_restore";
import { solveDelayZone$1 } from "./auto_routing";
import { acquireCombatMods$1, adjustForBanishIfPossible, adjustForBanishIfPossible$1, adjustForCopyIfPossible, adjustForFreeRunIfPossible, adjustForReplaceIfPossible, adjustForSniffingIfPossible, adjustForYellowRayIfPossible, auto_burningDelay, auto_burnMP, auto_change_mcd, auto_combat_appearance_rates, auto_convertDesiredML, auto_forceFreeRun, auto_gettingLucky, auto_have_skill, auto_haveQueuedForcedNonCombat, auto_interruptCheck$1, auto_is_valid, auto_log_debug, auto_log_debug$1, auto_log_error, auto_log_info, auto_log_info$1, auto_log_warning, auto_MaxMLToCap, auto_predictAccordionTurns, auto_queueIgnore, auto_setMCDToCap, auto_wantToBanish, auto_wantToBanish$1, auto_wantToCopy, auto_wantToFreeRun, auto_wantToReplace, auto_wantToSniff, auto_wantToYellowRay, executeFlavour, highest_available_mcd, internalQuestStatus, meatReserve, wrap_item } from "./auto_util";
import { auto_swoopLocations, is_ghost_in_zone, zone_combatMod, zone_needItem } from "./auto_zone";
...
import { auto_latteDropName, auto_latteDropWanted, isFantasyRealm, januaryToteAcquire } from "./iotms/mr2018";
import { auto_sausageGoblin, auto_snapperPreAdventure } from "./iotms/mr2019";
import { auto_handleRetrocape } from "./iotms/mr2020";
import { auto_backupTarget, auto_fireExtinguisherCharges, auto_FireExtinguisherCombatString, auto_forceHandleCrystalBall, auto_haveCrystalBall, auto_haveFireExtinguisher, have_fireworks_shop } from "./iotms/mr2021";
import { auto_handleParka, auto_haveCombatLoversLocket, auto_monsterInLocket, sweatpantsPreAdventure, utilizeStillsuit } from "./iotms/mr2022";
import { auto_allRifts, auto_availableBrickRift, auto_dousesRemaining, auto_handleCCSC } from "./iotms/mr2023";
import { auto_haveBatWings, auto_haveDarts, auto_swoopsRemaining, dartEleDmg } from "./iotms/mr2024";
import { auto_haveCupidBow, auto_havePeridot, auto_peridotSetZone, auto_wantSoCP, auto_wantToShrunkenHead$1, haveUsedPeridot$1, peridotManuallyDesiredMonsters } from "./iotms/mr2025";
import { ed_handleAdventureServant, edAcquireHP, edAcquireHP$1, isActuallyEd } from "./paths/actually_ed_the_undying";
...
import { in_plumber, plumber_equipTool$1, plumber_forceEquipTool } from "./paths/path_of_the_plumber";
...
function print_footer(): void
{
	...
	auto_log_info(`ML: ${monsterLevelAdjustment()} Encounter: ${combatRateModifier()} Init: ${initiativeModifier()}`, "blue");
	auto_log_info(`Exp Bonus: ${experienceBonus()} Meat Drop: ${meatDropModifier()} Item Drop: ${itemDropModifier()}`, "blue");
...
function auto_ghost_prep(place: Location): void
{
		...
		]))
	{
		...
		if (canUse$2(sk)) { //we can kill them with a spell
		return; }
	}
	...
	if (m_hot !== 0) { bonus += toInt(simValue("hot damage")); }
	if (m_cold !== 0) { bonus += toInt(simValue("cold damage")); }
	if (m_spooky !== 0) { bonus += toInt(simValue("spooky damage")); }
	if (m_sleaze !== 0) { bonus += toInt(simValue("sleaze damage")); }
	if (m_stench !== 0) { bonus += toInt(simValue("stench damage")); }
...
function auto_pre_adventure(): boolean
{
	...
	if (itemAmount(Item.get("handful of split pea soup")) === 0 && creatableAmount(Item.get("handful of split pea soup")) > 0)
	{
	...
	if (have_fireworks_shop() && itemAmount(
	// in a clan that has the Underground Fireworks Shop
		Item.get("red rocket")) === 0 && auto_is_valid(
		// Don't buy if we already have one
		Item.get("red rocket")) && canEat() && myClass() !== Class.get("Pig Skinner") && !in_wereprof() && !auto_haveDarts() && myMeat() > npcPrice(
		...
		Item.get("red rocket")) + meatReserve())
	{
		retrieveItem(1, Item.get("red rocket"));
	}

	if (getProperty("_bittycar") === "" && itemAmount(Item.get("BittyCar MeatCar")) > 0)
	{
		use(1, Item.get("BittyCar MeatCar"));
	}

	if (place === Location.get("The Broodling Grounds") && myClass() === Class.get("Seal Clubber"))
	{
		uneffect(Effect.get("Spiky Shell"));
		uneffect(Effect.get("Scarysauce"));
		if (!uneffect(Effect.get("Scariersauce"))) { abort("Could not uneffect [Scariersauce]"); }
	}

	if (myClass() === Class.get("Pastamancer") && myThrall() === Thrall.get("Vampieroghi") && place === Location.get("The Hidden Apartment Building") && auto_have_skill(Skill.get("Dismiss Pasta Thrall")))
	{
	...
	if (Location.get(["Next to that Barrel with Something Burning in it", "Near an Abandoned Refrigerator", "Over Where the Old Tires Are", "Out by that Rusted-Out Car"]).includes(place))
	{
		junkyardML = true;
		uneffect(Effect.get("Spiky Shell"));
		uneffect(Effect.get("Scarysauce"));
	...
	if (is_boris())
	{
		if (haveEffect(Effect.get("Song of Solitude")) === 0 && haveEffect(Effect.get("Song of Battle")) === 0)
		{
		...
		else if (place.turnsSpent > 1 && place !== toLocation(getProperty("auto_priorLocation")))
		{
			//When do we consider Song of Cockiness?
			buffMaintain$3(Effect.get("Song of Fortune"), 10, 1, 1);
			if (haveEffect(Effect.get("Song of Fortune")) === 0)
	...
	{
		if (auto_queueIgnore() || toMonster(getProperty("auto_nextEncounter")) !== Monster.none)
		{
	...
	{ //next encounter is a monster from the zone
		for (const [i, mon] of possible_monsters)
		{
			...
			if (auto_havePeridot() && peridotManuallyDesiredMonsters().has(mon) && !haveUsedPeridot$1(place))
			{
				zoneHasWantedMonsters = true;
			}
		}
	...
	// boris has no way to equip latte mug or Kramco (no offhand or familiar)
		if (auto_sausageGoblin() && place === solveDelayZone$1())
		{
			...
			if (canChangeToFamiliar(Familiar.get("Left-Hand Man")))
			{
				...
				auto_log_info(`We want to get the "${auto_latteDropName(place)}" ingredient for our Latte from ${place}, and we're buring delay using the Kramco so your Left-Hand Man will be bringing your Latte along!`, "blue"); handleFamiliar$1(Familiar.get("Left-Hand Man"));
	...
	{
		autoForceEquip$1(Slot.get("acc2"), wrap_item(Item.get("McHugeLarge left ski")));
	...
	else if (in_wildfire() && auto_haveFireExtinguisher() && place.fireLevel > 3)
	{
	...
	if (!haveUsedPeridot$1(place) && auto_havePeridot() && (zoneHasWantedMonsters || auto_peridotSetZone(place)))
	{
	...
	if (place === Location.get("The Penultimate Fantasy Airship") && auto_haveBatWings())
	{
	...
	if (in_plumber())
	{
		...
		let skip_equipping_flower: boolean = place === Location.get("The Haunted Billiards Room") && 18 <= pool_skill;
		...
		if (is_ghost_in_zone(place) && !skip_equipping_flower || place === Location.get("The Smut Orc Logging Camp") && possessEquipment(Item.get("frosty button")))
		{
		...
		{
			auto_log_debug$1("I expect to be flyering, equipping Pill Keeper to skip the first hit.");
	...
	if (is100FamRun() && myFamiliar() === Familiar.none)
	{
		...
		auto_log_debug$1(`Re-equipped your ${getProperty("auto_100familiar")} as something had unequipped it. This is bad and should be investigated.`);
	}

	if (Location.get(["Vanya's Castle", "The Fungus Plains", "Megalo-City", "Hero's Field"]).includes(place) && myTurncount() !== 0)
	{
	...
	if (place === Location.get("The Haunted Boiler Room") && myTurncount() !== 0 && internalQuestStatus("questL11Manor") < 3)
	{
	...
	if (Location.get(["Barrrney's Barrr", "The F'c'le", "The Poop Deck", "Belowdecks"]).includes(place)) {
	...
	else if (gettingLucky && !(Location.get(["The Hidden Temple", "The Red Zeppelin", "A Maze of Sewer Tunnels"]).includes(place))) {
	...
	if (auto_predictAccordionTurns() >= 8 && numericModifier(Modifier.get("MP Regen Min")) < 5)
	{
	...
	if ((toInt(getProperty("flyeredML")) > 9999 || internalQuestStatus("questL12War") > 1 || getProperty("sidequestArenaCompleted") !== "none") && myLevel() > 12)
	{
		doML = false;
		removeML = true;
		purgeML = false;
	...
	if (junkyardML && myBuffedstat(Stat.get("Moxie")) < 2 * monsterAttack(Monster.get("erudite gremlin")))
	{
		doML = false;
		removeML = true;
		purgeML = false;
	...
	if (doML)
	{
		...
		if (haveEffect(Effect.get("Driving Intimidatingly")) > 0 && toInt(getProperty("auto_debuffAsdonDelay")) >= 2)
		{
		...
		else if (toInt(haveEffect(Effect.get("Driving Intimidatingly"))) === 0 && toInt(getProperty("auto_debuffAsdonDelay")) >= 0)
		{
			setProperty("auto_debuffAsdonDelay", (0).toString());
		}
		else {
			setProperty("auto_debuffAsdonDelay", (toInt(getProperty("auto_debuffAsdonDelay")) + 1).toString());
			auto_log_debug$1(`Delaying debuffing Asdon: ${getProperty("auto_debuffAsdonDelay")}`);
		}
	...
	if (removeML)
	{
		...
		if (purgeML && itemAmount(Item.get("soft green echo eyedrop antidote")) > 5)
		{
		...
		else if (purgeML && isActuallyEd() && (itemAmount(Item.get("ancient cure-all")) > 0 || itemAmount(Item.get("soft green echo eyedrop antidote")) > 0))
		{
			uneffect(Effect.get("Blessing of Serqet"));
		}
	}
	...
	if (getProperty("auto_MLSafetyLimit") !== "" && !containsText(getProperty("auto_maximize_current"), "ml"))
	{
		...
		else if (toInt(getProperty("auto_MLSafetyLimit")) <= highest_available_mcd())
		{
	...
	if (!toBoolean(getProperty("auto_januaryToteAcquireCalledThisTurn")) && auto_is_valid(Item.get("wad of used tape")))
	{
	...
	if (place === Location.get("The Hidden Bowling Alley") && itemAmount(Item.get("bowling ball")) > 0 && toInt(getProperty("hiddenBowlingAlleyProgress")) < 5)
	{
		equipStatgainIncreasers$2();
		plumber_forceEquipTool();
	}
	else if (place === Location.get("The Haunted Ballroom") && internalQuestStatus("questM21Dance") === 3)
	{
		equipStatgainIncreasers$2();
		plumber_forceEquipTool();
	}
	else if (place === Location.get("The Shore, Inc. Travel Agency") && itemAmount(Item.get("forged identification documents")) === 0)
	{
	...
	if (isActuallyEd() && isWearingOutfit("Filthy Hippy Disguise") && place === Location.get("The Hippy Camp")) {
		...
		if (isWearingOutfit("Filthy Hippy Disguise")) {
			...
			auto_log_info$1("Took off the Filthy Hippy Disguise before adventuring in the Hippy Camp so we don't waste adventures on non-combats.");
		}
	}
	// Last minute debug logging and a final MCD tweak just in case Maximizer did silly stuff
	if (lowMLZones.includes(place))
	{
		auto_log_debug$1(`Going into a LOW ML ZONE with ML: ${monsterLevelAdjustment()}`);
	}
	else {
		...
		auto_log_debug$1(`Going into High or Standard ML Zone with ML: ${monsterLevelAdjustment()}`);
	}
	...
	if (Location.get(["A-Boo Peak"]).includes(place) && toInt(getProperty("auto_aboopending")) !== 0)
	{
		acquireHP();
		if (isActuallyEd())
		{
...
export function main(): void
{
	...
	try {
		...
		if (pathHasFamiliar() && canChangeFamiliar() && myFamiliar() === Familiar.none && !isFantasyRealm(myLocation())) {
