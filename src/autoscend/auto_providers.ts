...
import { auto_sourceTerminalEnhance, auto_sourceTerminalEnhanceLeft, rethinkingCandy } from "./iotms/mr2016";
import { asdonBuff$1, canAsdonBuff, horseCost, horseDark, horseMaintain, horseNone, isHorseryAvailable } from "./iotms/mr2017";
import { auto_latteRefill$5, januaryToteAcquire, songboomSetting } from "./iotms/mr2018";
import { auto_beachCombHead, auto_beachCombHeadEffect, auto_canBeachCombHead } from "./iotms/mr2019";
import { auto_birdModifier, auto_favoriteBirdModifier, auto_hasPowerfulGlove } from "./iotms/mr2020";
import { auto_haveCCSC } from "./iotms/mr2023";
import { auto_haveAprilingBandHelmet, auto_setAprilBandCombat, auto_setAprilBandNonCombat } from "./iotms/mr2024";
import { auto_haveCoolerYeti, auto_haveCyberRealm, beretBusk, canBusk } from "./iotms/mr2025";
...
import { bat_formBats, bat_formMist, bat_formWolf, bat_wantHowl } from "./paths/dark_gyffte";
...
export function providePlusCombat(amt: number, loc: Location, doEquips: boolean, speculative: boolean): number {
	...
	if (getProperty("_horsery") === "dark horse") {
		...
		auto_log_debug$1(`We ${(speculative ? "can remove" : "will remove")} Dark Horse, we will have ${result$4()}`);
	} else if (!speculative) {
		horseMaintain();
	}
	if (pass$4()) {
	...
	function handleEffect$4(eff: Effect): void {
		if (speculative) {
			...
			if (eff === Effect.get("Musk of the Moose") && haveEffect(Effect.get("Smooth Movements")) > 0) {
				delta += -1.0 * numericModifier(Effect.get("Smooth Movements"), "Combat Rate"); // numeric_modifier doesn't take into account uneffecting the opposite skill so we have to add it manually.
			}
			if (eff === Effect.get("Carlweather's Cantata of Confrontation") && haveEffect(Effect.get("The Sonata of Sneakiness")) > 0) {
				delta += -1.0 * numericModifier(Effect.get("The Sonata of Sneakiness"), "Combat Rate");
			}
		}
		auto_log_debug$1(`We ${(speculative ? "can gain" : "just gained")} ${eff.toString()}, now we have ${result$4()}`);
	}
	...
	// Cheap effects
	if (!speculative) { shrugAT$1(Effect.get("Carlweather's Cantata of Confrontation")); }
	if (tryEffects$5(new Map([
	...
	{ // meatpath only
		if (tryEffects$5(new Map([[Effect.get("Acting Jerky"), true]]))) { return result$4(); }
	}
	// Do the April band
...
export function providePlusCombat$2(amt: number, loc: Location, doEquips: boolean): boolean
{
...
export function providePlusNonCombat(amt: number, loc: Location, doEquips: boolean, speculative: boolean): number {
	...
	if (doEquips) {
		...
		delta = -1.0 * simValue("Combat Rate") - -1.0 * numericModifier("Combat Rate");
	...
	if (isHorseryAvailable() && myMeat() > horseCost() && getProperty("_horsery") !== "dark horse") {
		if (!speculative) {
			horseDark();
		}
		delta += -1.0 * numericModifier("Horsery:dark horse", "Combat Rate"); // horsery changes don't happen until pre-adventure so this needs to be manually added otherwise it won't count.
		auto_log_debug$1(`We ${(speculative ? "can gain" : "will gain")} Dark Horse, we will have ${result$5()}`);
	} else if (!speculative) {
		horseMaintain();
	}
	if (pass$5()) {
	...
	function handleEffect$5(eff: Effect): void {
		if (speculative) {
			...
			if (eff === Effect.get("Smooth Movements") && haveEffect(Effect.get("Musk of the Moose")) > 0) {
				delta += numericModifier(Effect.get("Musk of the Moose"), "Combat Rate"); // numeric_modifier doesn't take into account uneffecting the opposite skill so we have to add it manually.
			}
		}
		auto_log_debug$1(`We ${(speculative ? "can gain" : "just gained")} ${eff.toString()}, now we have ${result$5()}`);
	}
	...
	{
		if (tryEffects$6(new Map([[Effect.get("Darkened Meat"), true]]))) { return result$5(); }
	}

	if (!speculative) { shrugAT$1(Effect.get("The Sonata of Sneakiness")); }
	if (tryEffects$6(new Map([
	...
	if (-1.0 * auto_favoriteBirdModifier("Combat Rate") > 0) {
		if (tryEffects$6(new Map([[Effect.get("Blessing of your favorite Bird"), true]]))) {
			return result$5();
		}
	}
	// Do the April band
	...
	if (in_plumber() && 0 === haveEffect(Effect.get("Blooper Inked")) && itemAmount(Item.get("coin")) > 400) {
		if (!speculative) {
	...
	if (auto_hasPowerfulGlove() && tryEffects$6(new Map([[Effect.get("Invisible Avatar"), true]]))) {
		return result$5();
	}
...
export function providePlusNonCombat$1(amt: number, doEquips: boolean, speculative: boolean): number
{
...
export function providePlusNonCombat$3(amt: number, doEquips: boolean): boolean
{
...
export function provideInitiative(amt: number, loc: Location, doEquips: boolean, speculative: boolean): number
{
	...
	function handleEffect$1(eff: Effect): void
	{
		...
		auto_log_debug$1(`We ${(speculative ? "can gain" : "just gained")} ${eff.toString()}, now we have ${result$1()}`);
	}
	...
	{
		if (tryEffects$2(new Map([[Effect.get("Blessing of your favorite Bird"), true]])))
			{ return result$1(); }
	}

	if (doEquips && auto_have_familiar(Familiar.get("Grim Brother")) && haveEffect(Effect.get("Soles of Glass")) === 0 && toBoolean(getProperty("_grimBuff")) === false)
	{
		if (!speculative)
		{
	...
	if (auto_sourceTerminalEnhanceLeft() > 0 && haveEffect(Effect.get("init.enh")) === 0 && auto_is_valid$3(Effect.get("init.enh")))
	{
		if (!speculative)
	...
	if (doEquips && amt >= 400)
	{
		if (!toBoolean(getProperty("_bowleggedSwaggerUsed")) && buffMaintain$2(Effect.get("Bow-Legged Swagger"), 0, 1, 1, speculative))
		{
			if (speculative)
				{ delta += delta + numericModifier("Initiative"); }
			auto_log_debug$1(`With Bow-Legged Swagger we ${(speculative ? "can get to" : "now have")} ${result$1()}`);
		}
		if (pass$1())
			{ return result$1(); }
	}
...
export function provideInitiative$2(amt: number, loc: Location, doEquips: boolean): boolean
{
...
export function provideResistances(amt: Map<Element, number>, loc: Location, doEquips: boolean, doAll: boolean, speculative: boolean): Map<Element, number>
{
	...
	if ((amt.get(Element.get("stench")) ?? amt.set(Element.get("stench"), 0).get(Element.get("stench"))) > 0)
	{
	...
	{
		return toInt(numericModifier(`${ele} Resistance`) + (delta.get(ele) ?? delta.set(ele, 0).get(ele)) + (gearLoss.get(ele) ?? gearLoss.set(ele, 0).get(ele)));
	}
	...
	function handleEffect$6(eff: Effect): void
	{
		...
		auto_log_debug$1(`We ${(speculative ? "can gain" : "just gained")} ${eff.toString()}, now we have ${resultstring()}`);
	}
	...
	function pass$7(): boolean
	{
		...
		if (canChangeFamiliar() && Familiar.get(["Trick-or-Treating Tot", "Mu", "Exotic Parrot", "Cooler Yeti"]).includes(myFamiliar())) {
	...
	if (doEquips && canChangeFamiliar())
	{
		...
		if (resfam !== Familiar.none)
		{
			...
			if (auto_haveCoolerYeti() && amt.size === 1 && (amt.get(Element.get("cold")) ?? amt.set(Element.get("cold"), 0).get(Element.get("cold"))) > 0)
			{
				if ((resfam === Familiar.get("Mu") || resfam === Familiar.get("Exotic Parrot")) && floor((auto_famWeight$1(resfam) - 5) / 20 + 1) < floor(auto_famWeight$1(Familiar.get("Cooler Yeti")) / 11) || 5 < floor(auto_famWeight$1(Familiar.get("Cooler Yeti")) / 11))
				{
	...
	if (doAll)
	{
		...
		{
			if (tryEffects$7(new Map([
				[Effect.get("Cyber Resist x2000"), true]])))
				{ return result$7(); }
		}
		if (tryEffects$7(new Map([
...
function provideStats(amt: Map<Stat, number>, loc: Location, doEquips: boolean, speculative: boolean): Map<Stat, number>
{
	...
	function handleEffect$7(eff: Effect): void
	{
		...
		auto_log_debug$1(`We ${(speculative ? "can gain" : "just gained")} ${eff.toString()}, now we have ${resultstring$1()}`);
	}
	...
	else { //+10 mox. facial expression. 1 MP/adv
...
function provideMeat(amt: number, loc: Location, doEverything: boolean, speculative: boolean): number
{
	...
	if (doEverything && canChangeFamiliar())
	{
		...
		if (target !== Familiar.none && target !== myFamiliar())
		{
			...
			auto_log_debug$1(`With using familiar: ${target} we can get to ${result$3()}`);
		}
		else {
	...
	function handleEffect$3(eff: Effect): void
	{
		...
		auto_log_debug$1(`We ${(speculative ? "can gain" : "just gained")} ${eff.toString()}, now we have ${result$3()}`);
	}
	...
	{
		//Can be 20/40/60/80/100% meat drop
		if (tryEffects$4(new Map([[Effect.get("Blessing of your favorite Bird"), true]])))
			{ if (pass$3())
				{ return result$3(); } }
	}
	if (isActuallyEd())
	{
		//50% meat drop
		if (!haveSkill(Skill.get("Gift of the Maid")) && (Servant.get("Maid")).experience >= 441)
		{
	...
	if (availableAmount(Item.get("li'l pirate costume")) > 0 && canChangeToFamiliar(Familiar.get("Trick-or-Treating Tot")) && !in_heavyrains())
	{
	...
	if (auto_sourceTerminalEnhanceLeft() > 0 && haveEffect(Effect.get("meat.enh")) === 0 && auto_is_valid$3(Effect.get("meat.enh")))
	{
		if (!speculative)
	...
	if (itemAmount(Item.get("body spradium")) > 0 && !in_tcrs() && haveEffect(Effect.get("Boxing Day Glow")) === 0)
	{
	...
	if (doEverything)
	{
		...
		if (equippedItem(Slot.get("off-hand")) !== Item.get("Half a Purse") && !possessEquipment(Item.get("Half a Purse")) && itemAmount(Item.get("lump of Brituminous coal")) > 0)
		{
		...
		delta = simValue("Meat Drop") - numericModifier("Meat Drop");
		auto_log_debug$1(`With existing and crafted gear we can get to ${result$3()}`);
		if (pass$3())
			{ return result$3(); }
	}
	...
	if (doEverything)
	{
		...
		if (getProperty("sidequestArenaCompleted") === "fratboy" && !toBoolean(getProperty("concertVisited")) && haveEffect(Effect.get("Winklered")) === 0)
		{
			if (is_professor())
			{
		...
		if (pass$3())
			{ return result$3(); }
		if (!in_tcrs() && !in_small() && !toBoolean(getProperty("auto_limitConsume")) && haveEffect(Effect.get("Tryptofan")) === 0 && creatableAmount(Item.get("prize turkey")) > 0 && canEat$1(Item.get("prize turkey")) && stomach_left() > (Item.get("prize turkey")).fullness) {
			if (!speculative)
			{
				buy(Coinmaster.get("Skeleton of Crimbo Past"), 1, Item.get("prize turkey"));
		...
		if (!in_tcrs() && haveEffect(Effect.get("Grueling Gravitas")) === 0 && creatableAmount(Item.get("medicinal gruel")) > 0 && spleen_left() > (Item.get("medicinal gruel")).spleen) {
			if (!speculative)
			{
				buy(Coinmaster.get("Skeleton of Crimbo Past"), 1, Item.get("medicinal gruel"));
...
export function provideMeat$1(amt: number, doEverything: boolean, speculative: boolean): number
{
	return provideMeat(amt, myLocation(), doEverything, speculative);
}

export function provideMeat$2(amt: number, loc: Location, doEverything: boolean): boolean
{
...
function provideItem(amt: number, loc: Location, doEverything: boolean, speculative: boolean): number
{
	...
	if (doEverything && canChangeFamiliar())
	{
		...
		if (target !== Familiar.none && target !== myFamiliar())
		{
			...
			auto_log_debug$1(`With using familiar: ${target} we can get to ${result$2()}`);
		}
		else {
			auto_log_debug$1(`Already have desired familar, ${target}, active.`);
		}
	...
	function handleEffect$2(eff: Effect): void
	{
		...
		auto_log_debug$1(`We ${(speculative ? "can gain" : "just gained")} ${eff.toString()}, now we have ${result$2()}`);
	}
	...
	{
		if (tryEffects$3(new Map([[Effect.get("Beef Goggles"), true]]))) { // meatpath only
		return result$2(); }
	}
	// unlimited skills
	if (tryEffects$3(new Map([
	...
	{
		//Can be 10/20/30/40/50% item drop
		if (tryEffects$3(new Map([[Effect.get("Blessing of your favorite Bird"), true]])))
			{ return result$2(); }
	}
	// items
	if (tryEffects$3(new Map([
	...
	if (auto_is_valid(Item.get("possessed sugar cube")) && itemAmount(Item.get("possessed sugar cube")) > 0 && haveEffect(Effect.get("Dance of the Sugar Fairy")) === 0)
	{
		if (!speculative)
	...
	if (auto_sourceTerminalEnhanceLeft() > 0 && haveEffect(Effect.get("items.enh")) === 0 && auto_is_valid$3(Effect.get("items.enh")))
	{
		if (!speculative)
			{ //30% item
	...
	{
		//craft IOTM derivative that gives high item bonus
		if (!possessEquipment(Item.get("A Light that Never Goes Out")) && itemAmount(Item.get("lump of Brituminous coal")) > 0 && auto_is_valid(Item.get("A Light that Never Goes Out")))
		{
		...
		if (auto_is_valid(Item.get("broken champagne bottle")) && toInt(getProperty("garbageChampagneCharge")) > 0) {
		...
		delta = simValue("Item Drop") - numericModifier("Item Drop");
		auto_log_debug$1(`With existing and crafted gear we can get to ${result$2()}`);
	...
	{
		if (!toBoolean(getProperty("_steelyEyedSquintUsed")) && buffMaintain$2(Effect.get("Steely-Eyed Squint"), 0, 1, 1, speculative))
		{
			if (speculative)
				{ delta += delta + numericModifier("Item Drop"); }
			auto_log_debug$1(`With Steely Eyed Squint we ${(speculative ? "can get to" : "now have")} ${result$2()}`);
		}
		if (pass$2())
			{ return result$2(); }
	}
	...
	if (doEverything)
	{
		...
		if (pass$2())
			{ return result$2(); }
		if (zataraAvailable() && toBoolean(toInt(0 === haveEffect(Effect.get("There's No N in Love"))) & toInt(auto_is_valid$3(Effect.get("There's No N in Love")))))
		{
			if (!speculative)
			{
				zataraSeaside("item");
		...
		if (getProperty("sidequestArenaCompleted") === "hippy" && !toBoolean(getProperty("concertVisited")) && haveEffect(Effect.get("Dilated Pupils")) === 0)
		{
			if (is_professor())
			{
				//Need to manually equip because professor
		...
		if (pass$2())
			{ return result$2(); }
		if (!in_tcrs() && !in_small() && !toBoolean(getProperty("auto_limitConsume")) && haveEffect(Effect.get("Ordained")) === 0 && creatableAmount(Item.get("Smoking Pope")) > 0 && canDrink$1(Item.get("Smoking Pope")) && inebriety_left() > (Item.get("Smoking Pope")).inebriety) {
			if (!speculative)
			{
				buy(Coinmaster.get("Skeleton of Crimbo Past"), 1, Item.get("Smoking Pope"));
...
export function provideItem$2(amt: number, loc: Location, doEverything: boolean): boolean
{
...
export function provideFamExp(amt: number, loc: Location, doEquips: boolean, doEverything: boolean, speculative: boolean): number
{
	...
	if (doEquips || doEverything)
	{
		...
		delta = simValue("Familiar Experience") - numericModifier("Familiar Experience");
	...
	function handleEffect(eff: Effect): void
	{
		...
		auto_log_debug$1(`We ${(speculative ? "can gain" : "just gained")} ${eff.toString()}, now we have ${result()}`);
	}
	...
	if (in_zootomist() && myLevel() < 13) {
		if (tryEffects$1(new Map([[Effect.get("Milk of Familiar Kindness"), true], [Effect.get("Milk of Familiar Cruelty"), true]]))) {
			return result();
		}
	}
	// unlimited skills
	if (tryEffects$1(new Map([
	...
	if (doEverything)
	{
		...
		delta = simValue("familiar experience") - numericModifier("familiar experience");
		auto_log_debug$1(`With existing and crafted gear we can get to ${result()}`);
	...
	if (doEverything)
	{
		while (haveEffect(Effect.get("Blue Swayed")) < 31 && itemAmount(Item.get("pulled blue taffy")) > 0) {
		...
		if (fullness_left() > 2 && itemAmount(Item.get("roasted vegetable focaccia")) > 0)
		{
		...
		if (tryEffects$1(new Map([
			...
			{ if (pass())
				{ return result(); } }
		if (zataraAvailable() && toBoolean(toInt(0 === haveEffect(Effect.get("A Girl Named Sue"))) & toInt(auto_is_valid$3(Effect.get("A Girl Named Sue")))))
		{
			if (!speculative)
			{
		...
		if (auto_totalEffectWishesAvailable() === 1)
		{
			...
			for (const eff of Effect.get([
				...
				while (haveEffect(eff) === 0 || eff === Effect.get("Blue Swayed") && haveEffect(eff) < 31)
				{
					if (!speculative)
						{ success = auto_wishForEffect(eff); }
					specwishes += 1;
...
export function provideFamExp$2(amt: number, loc: Location, doEquips: boolean, doEverything: boolean): boolean
{
	return provideFamExp(amt, loc, doEquips, doEverything, false) >= amt;
}

export function provideFamExp$3(amt: number, doEquips: boolean, doEverything: boolean): boolean
{
