import { abort, advCost, appearanceRates, append, autosell, availableAmount, availableChoiceOptions, bufferToFile, buy, buyPrice, canadiaAvailable, canAdventure, canEquip, canInteract, changeMcd, charAt, chew, Class, cliExecute, cliExecuteOutput, Coinmaster, containsText, craft, create, currentMcd, Effect, Element, equip, equippedAmount, equippedItem, Familiar, floor, fullnessLimit, getCampground, getIngredients, getMonsters, getPlayerId, getProperty, gnomadsAvailable, guildStoreAvailable, haveCampground, haveEffect, haveEquipped, haveOutfit, haveSkill, inBadMoon, indexOf, inebrietyLimit, inHardcore, isTrendy, isUnrestricted, Item, itemAmount, itemDropModifier, itemType, knollAvailable, lastIndexOf, length, lightningCost, Location, max, maximize, min, Modifier, Monster, monsterElement, monsterLevelAdjustment, mpCost, myAdventures, myAscensions, myBasestat, myClass, myDaycount, myFamiliar, myFullness, myId, myInebriety, myLevel, myLightning, myLocation, myMeat, myMp, myPath, myPrimestat, myRain, mySign, mySoulsauce, mySpleenUse, myThunder, myTurncount, nowToInt, npcPrice, numericModifier, Path, Phylum, print, printHtml, pullsRemaining, rainCost, removeProperty, replaceString, retrieveItem, reverseNumberology, rollover, round, Servant, setLocation, setProperty, Skill, Slot, soulsauceCost, spleenLimit, splitString, squareRoot, Stat, storageAmount, substring, thunderCost, timeToString, toBoolean, toBuffer, toClass, todayToString, toEffect, toElement, toFamiliar, toFloat, toInt, toItem, toLocation, toLowerCase, toMonster, toPhylum, toSkill, toSlot, toStat, turnsPerCast, use, useFamiliar, useSkill, visitUrl } from "kolmafia";
...
import { addToMaximize, auto_loadEquipped, auto_saveEquipped, autoEquip, autoEquip$1, ensureSealClubs, equipMaximizedGear, equipmentAmount, possessEquipment, possessOutfit$1, removeFromMaximize } from "./auto_equipment";
...
import { auto_configureParka, auto_fightLocketMonster, auto_hasAutumnaton, auto_hasParka, auto_haveCombatLoversLocket, auto_haveGreyGoose, auto_haveTrainSet, auto_monsterInLocket, auto_ParkaSpikeForcesLeft, auto_voidMonster } from "./iotms/mr2022";
...
import { auto_equipAprilShieldBuff, auto_haveAprilShowerShield, auto_haveMcHugeLargeSkis, auto_McLargeHugeForcesLeft, auto_punchOutsLeft, auto_wantToBCZ } from "./iotms/mr2025";
...
{
	const warning_time: number = toInt(getProperty("auto_stopMinutesToRollover")) * 60;
...
{
	const defaults: Map<string, Map<string, string>> = fileAsMap("data/defaults.txt", [String, String, String]);
...
export function banishedMonsters(): Map<Monster, number>
{
	...
	const data: Map<number, string> = new Map(splitString(getProperty("banishedMonsters"), ":").map((_v, _i) => [_i, _v]));
...
function banishedPhyla(): Map<Phylum, number>
{
	...
	const data: Map<number, string> = new Map(splitString(getProperty("banishedPhyla"), ":").map((_v, _i) => [_i, _v]));
...
{ //return probability of fighting each monster if the encounter is not a noncombat
	...
	const res_including_noncombat: Map<Monster, number> = new Map(Object.entries(appearanceRates(place, queue)).map(([_k, _v]) => [Monster.get(_k), _v]));
	const res_excluding_noncombat: Map<Monster, number> = new Map();

	const noncombat_frequency: number = (res_including_noncombat.get(Monster.none) ?? res_including_noncombat.set(Monster.none, 0.0).get(Monster.none));
...
export function auto_banishesUsedAt(loc: Location): Map<string, boolean>
{
	function auto_reallyBanishesUsedAt(loc: Location): Map<string, boolean>
	{
		...
		const banishList: Map<number, string> = new Map(splitString(banished, ":").map((_v, _i) => [_i, _v]));
		const atLoc: Map<number, Monster> = new Map(getMonsters(loc).map((_v, _i) => [_i, _v]));
		const used: Map<string, boolean> = new Map();

		for (let i: number = 0; i + 1 < banishList.size; (i = i + 3))
		{
			const curMon: Monster = toMonster((banishList.get(i) ?? banishList.set(i, "").get(i)));
			const curUsed: string = (banishList.get(i + 1) ?? banishList.set(i + 1, "").get(i + 1));
	...
	if (Location.get(["Next to that Barrel with Something Burning in it", "Out by that Rusted-Out Car", "Over Where the Old Tires Are", "Near an Abandoned Refrigerator"]).includes(loc))
	{
		...
		for (const l of Location.get(["Next to that Barrel with Something Burning in it", "Out by that Rusted-Out Car", "Over Where the Old Tires Are", "Near an Abandoned Refrigerator"]))
		{
...
export function freeRunCombatString(enemy: Monster, loc: Location, inCombat: boolean): string
{
	...
	if (!inAftercore() && haveEffect(Effect.get("Everything Looks Green")) === 0)
	{
		...
		for (const it of Item.get(["green smoke bomb", "tattered scrap of paper", "GOTO"]))
		{
	...
	if (canChangeToFamiliar(Familiar.get("Frumious Bandersnatch")))
	{
		...
		const banderRunsLeft: number = floor(auto_famWeight$1(Familiar.get("Frumious Bandersnatch")) / 5) - toInt(getProperty("_banderRunaways"));
	...
	if (canChangeToFamiliar(Familiar.get("Pair of Stomping Boots")))
	{
		...
		const banderRunsLeft: number = floor(auto_famWeight$1(Familiar.get("Pair of Stomping Boots")) / 5) - toInt(getProperty("_banderRunaways"));
		if (is_professor()) { return ""; }
		if (!inCombat)
		{
	...
	const potential_split_pea_soup: number = availableAmount(Item.get("whirled peas")) / 2 + availableAmount(Item.get("handful of split pea soup"));
...
export function adjustForYellowRayIfPossible(target: Monster): boolean
{
	...
	{
		const yr_string: string = yellowRayCombatString(target, false, Monster.get(["bearpig topiary animal", "elephant (meatcar?) topiary animal", "spider (duck?) topiary animal", "knight (Snake)"]).includes(target));
...
export function banishSources(): number
{
	...
	let count_1: number = 0;
	for (const sk of Skill.get(["Peel Out", "Howl of the Alpha", "Baleful Howl", "Thunder Clap", "Curse of Vacation", "Breathe Out", "Batter Up!",
	...
	//equipment
	for (const eq of Item.get(["spring shoes", "latte lovers member's mug", "stinky cheese eye", "V for Vivala mask", "mafia middle finger ring", "Pantsgiving",
	...
	for (const it of Item.get(["cosmic bowling ball", "stuffed yam stinkbomb", "handful of split pea soup", "human musk",
...
export function freeRunSources(): number
{
	...
	//equipment
	for (const eq of Item.get(["spring shoes", "Roman Candelabra", "navel ring of navel gazing", "replica navel ring of navel gazing"]))
	{
		if (possessEquipment(eq) && auto_can_equip(eq))
		{
			count_1 += 1;
	...
	//combat items/IOTMs/IOTM-Derived items that aren't equipment
	for (const it of Item.get(["green smoke bomb", "tattered scrap of paper", "GOTO", "cosmic bowling ball", "handful of split pea soup", "giant eraser"]))
	{
		if (auto_is_valid(it) && itemAmount(it) > 0)
		{
			count_1 += 1;
	...
	//familiars
	for (const fam of Familiar.get(["Frumious Bandersnatch", "Pair of Stomping Boots"]))
	{
		if (auto_have_familiar(fam) && canChangeToFamiliar(fam))
		{
			count_1 += 1;
...
export function freeKillSources(): number
{
	...
	let count_1: number = 0;
	for (const sk of Skill.get(["Lightning Strike", "Shattering Punch", "Gingerbread Mob Hit", "Free-For-All"]))
	{
		if (auto_have_skill(sk))
		{
			count_1 += 1;
	...
	//equipment
	for (const eq of Item.get(["blood cubic zirconia", "legendary seal-clubbing club", "Everfull Dart Holster", "Lil' Doctor&trade; bag", "The Jokester's gun"]))
	{
		if (possessEquipment(eq) && auto_can_equip(eq))
		{
			count_1 += 1;
	...
	//combat items/IOTMs/IOTM-Derived items that aren't equipment
	for (const it of Item.get(["power pill", "groveling gravel", "replica bat-oomerang", "shadow brick"]))
	{
		if (auto_is_valid(it) && itemAmount(it) > 0)
		{
			count_1 += 1;
...
export function yellowRaySources(): number
{
	...
	let count_1: number = 0;
	for (const sk of Skill.get(["Fondeluge", "Disintegrate", "Ball Lightning", "Wrath of Ra", "Flash Headlight", "Unleash Cowrruption", "Feel Envy",
	"Shocking Lick"]))
	{
		if (auto_have_skill(sk))
		{
	...
	//equipment
	for (const eq of Item.get(["Jurassic Parka", "Roman Candelabra", "unwrapped knock-off retro superhero cape",
	...
	//combat items/IOTMs/IOTM-Derived items that aren't equipment
	for (const it of Item.get(["yellowcake bomb", "yellow rocket", "spitball", "Golden Light", "pumpkin bomb", "unbearable light",
	...
	//campground equipment
	for (const it of Item.get(["portable Mayo Clinic", "Asdon Martin keyfob (on ring)"]))
	{
		if (have_workshed() && auto_get_campground().has(it))
		{
			count_1 += 1;
...
export function copySources(): number
{
	...
	let count_1: number = 0;
	for (const sk of Skill.get(["Macrometeorite", "Recall Facts: Monster Habitats", "Digitize", "Rain Man"]))
	{
		if (auto_have_skill(sk))
		{
			count_1 += 1;
	...
	//equipment
	for (const eq of Item.get(["Powerful Glove", "backup camera", "Roman Candelabra"]))
	{
		if (possessEquipment(eq) && auto_can_equip(eq))
		{
			count_1 += 1;
	...
	//combat items/IOTMs/IOTM-Derived items that aren't equipment
	for (const it of Item.get(["waffle", "Rain-Doh black box", "Time-Spinner", "combat lover's locket"]))
	{
		if (auto_is_valid(it) && itemAmount(it) > 0)
		{
			count_1 += 1;
...
export function sniffSources(): number
{
	...
	// Meat Cute: Skill
	let count_1: number = 0;
	for (const sk of Skill.get(["Transcendent Olfaction", "Make Friends", "Hunt", "Long Con", "Perceive Soul", "Motif",
	...
	//equipment
	for (const eq of Item.get(["cursed monkey's paw", "McHugeLarge left pole", "latte lovers member's mug"]))
	{
		if (possessEquipment(eq) && auto_can_equip(eq))
		{
			count_1 += 1;
...
export function LX_summonMonster(): boolean
{
	...
	const gunpowder_left: number = 5 - itemAmount(Item.get("barrel of gunpowder"));
...
export function fightScienceTentacle(): boolean
{
	...
	const choices: Map<number, string> = new Map(Object.entries(availableChoiceOptions()).map(([_k, _v]) => [toInt(_k), _v]));
...
export function handleBarrelFullOfBarrels(daily: boolean): boolean
{
	...
	const mimic_matcher: AshMatcher = new AshMatcher("<div class=\"ex\">((?:<div class=\"mimic\">!</div>)|)<a class=\"spot\" href=\"choice.php[?]whichchoice=1099[&]pwd=(?:.*?)[&]option=1[&]slot=(\\d\\d)\"><img title=\"(.*?)\"", page);
...
export function use_barrels(): boolean
{
	...
	const barrels: Item[] = Item.get(["little firkin", "normal barrel", "big tun", "weathered barrel", "dusty barrel", "disintegrating barrel", "moist barrel", "rotting barrel", "mouldering barrel", "barnacled barrel"]);
...
export function runChoice$1(page_text: string): string
{
	while (containsText(page_text, "choice.php"))
	{
		...
		const begin_choice_adv_num: number = indexOf(page_text, "whichchoice value=") + 18;
		const end_choice_adv_num: number = indexOf(page_text, "><input", begin_choice_adv_num);
		const choice_adv_num: string = substring(page_text, begin_choice_adv_num, end_choice_adv_num);
...
function doNumberology$3(goal: string, doIt: boolean, option: CombatMacro): number
{
	...
	const numberology: Map<number, number> = new Map(Object.entries(reverseNumberology()).map(([_k, _v]) => [toInt(_k), _v]));
...
export function candyEggDeviler(): boolean
{
	...
	{
		for (const ut of Item.get(["Comet Pop", "black candy heart", "explosion-flavored chewing gum"]))
		{
...
{
	for (const sk of Skill.get(["Summon Crimbo Candy", "Summon Candy Heart", "Chubby and Plump", "Summon Hilarious Objects"]))
	{
...
export function auto_get_campground(): Map<Item, number>
{
	...
	const campItems: Map<Item, number> = new Map(Object.entries(getCampground()).map(([_k, _v]) => [Item.get(_k), _v]));
	...
	if (in_nuclear() && !$_auto_get_campground_didCheck)
	{
		...
		const temp: string = visitUrl("place.php?whichplace=falloutshelter&action=vault_term");
...
export function auto_check_conditions(conds: string): boolean
{
	...
	const conditions: Map<number, string> = new Map(splitString(conds, ";").map((_v, _i) => [_i, _v]));
...
function auto_getMonsters(category: string): Map<Monster, boolean>
{
	...
	const monsters_text: Map<string, Map<number, Map<string, string>>> = fileAsMap("autoscend_monsters.txt", [String, Number, String, String]);
	if (!monsters_text.size)
		{ auto_log_error("Could not load autoscend_monsters.txt. This is bad!"); }
	for (const [i, _v0] of (monsters_text.get(category) ?? monsters_text.set(category, new Map()).get(category))) {
		for (const [name, _v1] of _v0) {
			const conds = _v1;
...
function auto_getPhylum(category: string): Map<Phylum, boolean>
{
	...
	const phylum_text: Map<string, Map<number, Map<string, string>>> = fileAsMap("autoscend_phylums.txt", [String, Number, String, String]);
	if (!phylum_text.size)
		{ auto_log_error("Could not load autoscend_phylums.txt. This is bad!"); }
	for (const [i, _v0] of (phylum_text.get(category) ?? phylum_text.set(category, new Map()).get(category))) {
		for (const [name, _v1] of _v0) {
			const conds = _v1;
...
function auto_interruptZoneCheck(): boolean
{
	...
	const interruptedZones: string = toBuffer(getProperty("auto_interruptedZones"));
...
if (getProperty("walfordBucketItem") === "ice" && equippedItem(Slot.get("off-hand")) === Item.get("Walford's bucket"))
	...
	for (const ele of Element.get(["cold", "hot", "sleaze", "spooky", "stench", "none"]))
	{
	...
	function handle_monster(mon: Monster, chance: number): void
	{
		...
		for (const ele of Element.get(["cold", "hot", "sleaze", "spooky", "stench"]))
		{
	...
	for (const [mon, chance] of Object.entries(appearanceRates(place)).map(([_k, _v]) => [Monster.get(_k), _v] as [Monster, number]))
	{
	...
	for (const ele of Element.get(["cold", "hot", "sleaze", "spooky", "stench"]))
	{
		const spellDamage: number = numericModifier(`${ele.toString()} Spell Damage`);
...
export function canSimultaneouslyAcquire(needed: Map<Item, number>): boolean
{
	...
	{
		const needToCraft: number = (alreadyUsed.get(toAdd) ?? alreadyUsed.set(toAdd, 0).get(toAdd)) + amount - itemAmount(toAdd);
		...
		if (needToCraft > 0)
		{
			...
			for (const [ing, ingAmount] of Object.entries(getIngredients(toAdd)).map(([_k, _v]) => [Item.get(_k), _v] as [Item, number]))
			{
...
{
	const itemdata: Map<string, Map<number, Map<string, string>>> = fileAsMap("autoscend_items.txt", [String, Number, String, String]);
	if (!itemdata.size)
		{ auto_log_error("Could not load autoscend_items.txt! This is bad!"); }
	for (const [i, _v0] of (itemdata.get("reserve") ?? itemdata.set("reserve", new Map()).get("reserve"))) {
...
export function auto_reserveCraftAmount(orig_it: Item): number
{
	...
	function inner(it: Item): number
	{
		...
		for (const [ing, amt] of Object.entries(getIngredients(it)).map(([_k, _v]) => [Item.get(_k), _v] as [Item, number]))
		{
...
export function currentPoolSkill(): number {
	...
	const poolskill_command: Map<number, string> = new Map(splitString(cliExecuteOutput("poolskill")).map((_v, _i) => [_i, _v]));
...
export function hasUsefulShirt(): boolean
{
	...
	for (const it of Item.get(["January's Garbage Tote", "astral shirt", "shoe ad T-shirt", "fresh coat of paint", "tunac", "Jurassic Parka", "hairshirt", "futuristic shirt"]))
	{
...
export function auto_burnMP(mpToBurn: number): boolean
{
	...
	for (const sk of Skill.get(["Sauce Contemplation", "Seal Clubbing Frenzy", "Patience of the Tortoise", "Manicotti Meditation", "Disco Aerobics", "Moxie of the Mariachi"]))
	{
		if (haveSkill(sk))
		{
			defaultSkill = sk;
...
export function auto_getListOfNonDamagingFamiliarEquipment(): Map<number, Item>
{
	...
	const base_list: Item[] = Item.get(["astral pet sweater", "tiny stillsuit", "tiny gold medal", "lead necklace",
...
{
	let turns: number = 0;
	for (const ef of Effect.get(["Who's Going to Pay This Drunken Sailor?", "Only Dogs Love a Drunken Sailor",
