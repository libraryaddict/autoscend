...
type Schema<T = any> = StringConstructor | NumberConstructor | ((raw: string) => T) | CtorLeaf<T>;

type Value<C> =
    C extends CtorLeaf<infer T> ? T :
    C extends StringConstructor ? string :
    C extends NumberConstructor ? number :
    C extends (raw: string) => infer T ? T :
    never;

// One Map<key, ...> layer per schema entry; the last entry is the leaf value, not a key.
type NestedMap<S extends readonly Schema[]> =
    S extends readonly [infer Only extends Schema] ? Value<Only> :
    S extends readonly [infer Head extends Schema, ...infer Rest extends Schema[]] ? Map<Value<Head>, NestedMap<Rest>> :
    never;

// Replacement for fileToMap, could've returned tuples, but eh.
export function fileAsMap<const S extends readonly [Schema, Schema, ...Schema[]]>(filename: string, schema: S): NestedMap<S> {
    ...
    for (const line of fileToBuffer(filename).split(/\r?\n/)) {
        ...
        for (let i = 0; i < depth; i++) {
            ...
            current.set(key, leaf instanceof CtorLeaf
...
export class AshMatcher {
    ...
    group(group: number | string = 0) {
        ...
        if (typeof group === 'number') return (this.match as any)[group] !== undefined ? (this.match as any)[group] : "";
        return this.match.groups && this.match.groups[group] !== undefined ? this.match.groups[group] : "";
    }
    ...
    start(group: number = 0) { return this._span(group)[0]; }
    ...
    replaceFirst(replacement: string) {
        const nonGlobal = new RegExp(this.pattern.source, this.pattern.flags.replace('g', ''));
