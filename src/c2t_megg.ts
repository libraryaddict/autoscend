...
function c2t_megg_donate$1(): boolean {
  return c2t_megg_donate(Monster.none);
}
//extract egg
//fight egg
...
function c2t_megg_success$1(): boolean {
...
function main$c2t_megg(...args: string[]): void {
  ...
  if (args.length === 1) {
    split = new Map(
      splitString((args[0] ??= ""), " ").map((_v, _i) => [_i, _v]),
    );
  ...
  switch (action) {
    ...
    case "help":
      ...
      print(
        "c2t_megg donate [monster] -- used to donate mimic eggs of monster, or random if monster omitted",
      );
      print(
        "c2t_megg eggs -- prints list of eggs on hand and their quantities, as read the preference containing mimic egg contents",
      );
      print(
        "c2t_megg extract <monster> -- used to extract mimic egg of monster from Mimic DNA Bank",
      );
      print(
        "c2t_megg fight <monster> -- enter combat with monster contained in a mimic egg",
      );
      print(
        "c2t_megg maxed -- prints list of monsters that are maxed, as read from the data file",
      );
      print(
        "c2t_megg preadv -- updates the maxed egg list if able, but with time restictions useful for pre-adventure scripts",
      );
      print("c2t_megg update -- updates the maxed egg list if able");
      print(
        "c2t_megg relay <on|off> -- turn the relay cleaner on or off; the cleaner changes monster names to use mafia's names for them and makes the drop-down menus searchable when visiting the Mimic DNA Bank",
      );
...
function c2t_megg_donate(target: Monster): boolean {
  ...
  if (target !== Monster.none) {
    ...
    do {
      ...
      page = visitUrl(
        `choice.php?pwd&whichchoice=1517&option=1&mid=${monstring}`,
        true,
        true,
      );
    } while (!(
      itemAmount(egg) === 0 ||
      toInt(getProperty(pref)) >= 3 ||
      ++tries >= MAX_TRIES
    ));
  } else {
    //random donation
    do {
      options = new Map(
        xpath(
          page,
          `//form[@action="choice.php"][${numForms}]/select/option/@value`,
        ).map((_v, _i) => [_i, _v]),
      );
      size = options.size;
      ...
      if (roundtrip) {
        return c2t_megg_success(
          "all eggs you have appear to be max donated already",
        );
      }
    } while (!(
      itemAmount(egg) === 0 ||
      toInt(getProperty(pref)) >= 3 ||
      ++tries >= MAX_TRIES
    )); //just in case logic is off or there is a desync with mafia, don't let this loop infinitely
  }
  //result
  if (toInt(getProperty(pref)) >= 3) {
  ...
  if (tries >= MAX_TRIES) {
    return c2t_megg_error(
      `mafia out of sync? tried to donate ${tries} times without fully succeeding`,
    );
  }
  //something happened that I didn't think of; or something to do with protection
  c2t_megg_print(
    `${getProperty(pref)},${itemAmount(egg)},${tries},${monstring},${options.size}`,
  );
...
function c2t_megg_fight(target: Monster): boolean {
  ...
  page = visitUrl(
    `choice.php?pwd&whichchoice=1516&option=1&mid=${monstring}`,
    true,
    true,
  );
...
function c2t_megg_update(): boolean {
  ...
  if (dailyMaxed) {
    return c2t_megg_error(
      "daily max eggs obtained; can't update list anymore today",
    );
  }
  //go
  useFamiliar(mimic);
...
function c2t_megg_relay(page: string): string {
  ...
  while (m.find()) {
    replaceString(
      buf,
      m.group(0),
      `${m.group(1)}${toMonster(m.group(2)).name}`,
    );
  }
  ...
  while (m.find()) {
    if (maxlist.has(m.group(2))) {
      replaceString(
        buf,
        `${m.group(1)}${m.group(3)}`,
        `${m.group(1)} disabled${m.group(3)} (max)`,
      );
    }
  }
  ...
  replaceString(
    buf,
    "</head>",
    '<script src="https://code.jquery.com/jquery-3.7.1.min.js"></script><link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" /><script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script><script type="text/javascript">var jQuery_3_7_1 = $.noConflict(true);jQuery_3_7_1(document).ready(function() {jQuery_3_7_1(\'.searchable-select\').select2();});</script></head>',
  );
  replaceString(
    buf,
    '<select name="mid">',
    '<select class="searchable-select" name="mid">',
  );
