...
export class Args {
   ...
   */
  static showHelp<T extends ArgMap>(
    args: ParsedArgs<T>,
  ...
  static showGroupHelp<T extends ArgMap, S extends ArgMap>(
    metadata: WrappedArgMetadata<T>,
    group: ArgGroup<S>,
    key: string,
  ) {
    printHtml("");
  ...
  static showArgHelp<T extends ArgMap, S>(
    metadata: WrappedArgMetadata<T>,
    arg: Arg<S> | ArgNoDefault<S>,
    key: string,
    maxOptionsToDisplay?: number,
  ) {
   ...
   */
  static getMetadata<T extends ArgMap>(
    args: ParsedArgs<T>,
  ): WrappedArgMetadata<T> {
...
export type ParsedArgs<T extends ArgMap> = ParsedGroup<T> & ArgMetadata<T>;
    ...
    while (!this.finished()) {
      ...
      if (this.flags.has(lowerKey)) {
        ...
        if (this.peek() === "=") {
          ...
          result.set(
            resolvedKey,
            parsing_negative_flag ? flag_char + value : value,
          );
        } else {
